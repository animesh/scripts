...
df['DP Mass Difference'].hist()

df['DP Modification'].value_counts().plot(kind='bar').figure.savefig('foo.png',dpi=100,bbox_inches = "tight")

dfDP=df.loc[:, df.columns.str.startswith('DP')]
dfDP.rename(columns = lambda x : str(x)[3:])
dfDP.to_csv('dfDP.csv')
