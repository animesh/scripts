...
CrossTable(mydata$myrowvar, mydata$myIolvar)

} tapply(iriJ$Jepal.Length,iriJ$JpecieJ,mean)
...
Recognize and treat missing ualues and outliers

} y <- c(4,S,6,NA)
} iJ.na(y)
[I] FALSE FALSE FALSE TRUE

# and here is a quick ix or the Jame

} y[is.na(y)] c- mean(y,na.rm=TRUE)
} y[is.na(y)] c- mean(y,na.rm=TRUE)
...
Merge l Join data sets

# merge two data rames by ID
total - merge(datarameA,data_rrame8,by="ID

# merge two data rrameJ by ID and Country
} total c - merge(data_rrameA,data_rrame8,by=c("ID","Country")
} total <- rbind(data_rrameA, data_frameB
