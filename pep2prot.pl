...
while(my $l=<F>){
	...
	for(my $cnt1;$cnt1<=$#tpep;$cnt1++){
		#for(my $cnt2;$cnt2<=$#tmod;$cnt2++){
			...
			while ($start>0){ #overlapping matches
				#if($tmodpos[$c4-1]=="MOD_RES"&&$tmodpos[$c4]<=$end&&$tmodpos[$c4]>=$start){
					#print "$ln\t$cnt1\t$cnt2\t$start\t$end\t$tpep[$cnt1]\t$tmod[$cnt2]\n";
				#}
				print "$ln\t$cnt1\t$start\t$end\t$tpep[$cnt1]\t$t[$c2]\n";
				$start=index($prot, $tpep[$cnt1], $start)+1; # single match
				$end = $start+length($tpep[$cnt1])-1;
			}
		#}
	}
}
