#C:\Users\animeshs\R-4.2.3\bin\Rscript.exe peptideGroups.r C:\Users\animeshs\230504_hela_test\combined\txtMC1
print("USAGE:Rscript peptideGroups.r <complete path to proteinGroups.txt file> <LFQ or SILAC if performed else it defaults to raw Intensity columns>")
#supplying input file for testing
#inpF<-file.path("L:/promec/HF/Lars/2023/230331_MOHNSEN/230328_mohsen_collagen_1_PeptideGroups.txt")
#parse argument(s)0
...
if(sum(grep(selection,colnames(data)))>0){
  ...
  intensity<-as.matrix(data[,grep(selection,colnames(data))]);protNum<-1:nrow(data);row.names(data)<-paste(protNum,data$Fasta.headers,protNum,sep=";")}  else if(sum(grep("Abundance.",colnames(data)))>0){selection<-"Abundance.";intensity<-as.matrix(data[,grep("Abundance.",colnames(data))]);protNum<-1:nrow(data);row.names(data)<-paste(protNum,data$Protein.Accessions,data$Master.Protein.Descriptions,data$Positions.in.Proteins,data$Sequence,sep=";")}  else{print('Neither Abundance[PD] nor Intensity[MQ] columns detected!')}
...
dim(log2LFQ)
#RT####
log2LFQretention<-cbind(log2LFQ,data$RT.in.min.by.Search.Engine.Sequest.HT)
colnames(log2LFQretention)[ncol(log2LFQretention)]<-"RT"
colnames(log2LFQretention)
#plot(log2LFQretention[,"RT"],log2LFQretention[,1])
#hist(log2LFQretention)
for(i in colnames(log2LFQretention)){
  #i="2_60_2"
  print(i)
  plot(log2LFQretention[,"RT"],log2LFQretention[,i],ylab=i, xlab="RT", pch=18,  cex=0.5, col="black",ylim = c(min(log2LFQ,na.rm=T),max(log2LFQ,na.rm=T)))
  text(log2LFQretention[,"RT"],log2LFQretention[,i],data$Sequence, cex=0.3, pos=2, col="red")
  #hist(log2LFQretention[,i])
  Significance=log2LFQretention[,i]>25
  Significance[is.na(Significance)]<-FALSE
  #plot(Significance)
  print(sum(Significance))
  #dsub <- subset(log2LFQretention,Significance)
  #p <- ggplot2::ggplot(log2LFQretention,ggplot2::aes(i,"RT")) + ggplot2::geom_point(ggplot2::aes(color=Significance))
...
#ggplot2::ggplot(data = log2LFQretention), ggplot2::aes(x=x, y=value))+ ggplot2::geom_line(ggplot2::aes(colour=variable))
