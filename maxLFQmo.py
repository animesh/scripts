...
@app.cell
def _(mo, peptide_df, protein_groups_df, pep_err, pg_err):
    _out = mo.md("")
    if pep_err:
...
def _(peptide_df, first_col):
    ...
    return charge_col, intensity_cols, mod_col, protein_col, samples, sequence_col


@app.cell
def _(mo, protein_col, sequence_col, mod_col, charge_col, intensity_cols):
    _out = mo.md("")
...
# ── Clean + build species matrix ──────────────────────────────────────────────
@app.cell
def _(peptide_df, protein_col, sequence_col, mod_col, charge_col, intensity_cols, samples, first_token, pd, np):
    if peptide_df is None or protein_col is None:
        species_matrix = None
    else:
        df = peptide_df.copy()
        for flag in ["Reverse", "Potential contaminant", "Contaminant"]:
...
@app.cell
def _(np, pd, samples, N_scale_free, normalization_residuals, anchor_sample):
...
# ── Apply normalization → Eq. 1 ───────────────────────────────────────────────
@app.cell
def _(species_matrix, normalization_table):
...
def _(mo, protein_selector, normalized_species_matrix, protein_list, px):
    _out = mo.md("")
    if protein_list and normalized_species_matrix is not None and protein_selector.value:
        prot = protein_selector.value
        psm = normalized_species_matrix.loc[prot].copy()
        ...
        _out = mo.vstack([
            px.bar(long_df, x="Sample", y="Intensity", color="_species", log_y=True,
                   title=f"{prot} — normalized species intensities"),
...
@app.cell
def _(np, pd, samples, normalized_species_matrix, rescale_method, anchor_sample, 
      combinations, minimum_ratio_count, ratio_method):
    if normalized_species_matrix is None:
        calculated_lfq = None
    else:
        calc_rows = []
        ...
        for protein2, pdf2 in normalized_species_matrix.groupby(level=0, sort=True):
            ...
            logm2 = np.log2(mat)
            ratio_mat = np.full((ns2, ns2), np.nan)
            for j2, k2 in combinations(range(ns2), 2):
...
def _(protein_groups_df, calculated_lfq, first_token, pd, np, samples):
    if protein_groups_df is None or calculated_lfq is None:
        ...
        for flag2 in ["Reverse", "Potential contaminant", "Contaminant"]:
...
def _(mo, samples):
    ...
    return anchor_sample, minimum_ratio_count, ratio_method, rescale_method
