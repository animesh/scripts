```{r data}
library(readxl)
data<-read_excel("F:/HeLa/ayu/For prediction modeling.xlsx", sheet=1)
summary(data)
```

```{r cycle}
tp<-read_excel("F:/HeLa/ayu/For prediction modeling.xlsx", sheet=3)
summary(tp)
```
...
source("L:/promec/Animesh/JTKversion3/JTK_CYCLEv3.1.R")
```
