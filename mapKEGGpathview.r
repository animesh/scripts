...
inpF1 <- if (length(args) >= 1 && nzchar(args[1])) args[1] else "L:/promec/TIMSTOF/LARS/2023/230217_Caroline/combined/txt/proteinGroups.txtLFQ.intensity.110Omego1Cntr1h00.050.5InfBiotTestBH.xlsx.110Omego3Cntr3h00.110Omego6Cntr6h00.110Omego12Cntr12h00.Log2MedianChange.RowGeneUniProtScorePepsselect.xlsx"
pathway <- if (length(args) >= 2 && nzchar(args[2])) strsplit(args[2], ",")[[1]] else c("hsa05135","hsa04140")
idPos <- if (length(args) >= 3 && nzchar(args[3])) args[3] else 3
scale <- if (length(args) >= 3 && nzchar(args[3])) args[3] else 5
# read table
...
allNE<-AnnotationDbi::select(org.Hs.eg.db, keys = allN, keytype = 'UNIPROT', columns = c('ENTREZID'))
#allNE[is.na(allNE$ENTREZID), 'ENTREZID'] <- allNE[is.na(allNE$ENTREZID), 'UNIPROT']
matRepID<-merge(matRep, allNE, by.x=0, by.y='UNIPROT', all=TRUE)
...
hda <- aggMat[,grep("Clip", colnames(aggMat), fixed=TRUE)]
write.csv(hda, file=paste0(tools::file_path_sans_ext(basename(inpF1)), ".IDmap.absmax.csv"), row.names=TRUE)
#hda[is.na(hda)] <- 0
#hda<-unique(hda)
#hda <- scales::rescale(as.matrix(hda), c(-1*scale, scale))
#pathview(hda, pathway.id = "04140", species = "hsa", gene.idtype = "ENTREZID")
...
for (p in pathway) {
  ...
  pv <- try(pathview(gene.data = hda, pathway.id = p, species = "hsa", gene.idtype = "ENTREZID",low = list(gene = "blue"), mid = list(gene = "white"), high = list(gene = "orange"),both.dirs = list(gene = TRUE), na.col = "grey",limit = list(gene = max(abs(hda), na.rm = TRUE))), silent = TRUE)
