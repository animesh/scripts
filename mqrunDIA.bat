...
:: For each folder: copies 3 data files to TMPDIR, writes mqparDIA.xml,
...
:: the given mqparDIA.xml path. Sets ADOPT_PID if found, clears it otherwise.
:: Arg1: full path to mqparDIA.xml
:AdoptPID
set "ADOPT_PID="
set "_AP=%~1"
set "_AP=!_AP:\=\\!"
...
if "!MD_PID!"=="" (
    :: No PID recorded -- check if any live MQ is using this mqparDIA.xml
...
exit /b 0
:: :WriteParam -- rewrites template mqparDIA.xml with job-specific paths.
...
:: Arg1: DATADEST path   Arg2: FASTAFILE path   Arg3: destination mqparDIA.xml path
:WriteParam
...
if !NEED_PARAM! EQU 1 (
    ...
    if not exist "!RUNPARAM!" ( set /a ERRORS+=1 & call :Log "ERROR    : %BN% mqparDIA.xml write failed" & exit /b 0 )
)
:: -- Launch gate --------------------------------------------------------------
:: If no slot available, save queued.lock and exit. Next poll will resume here
:: (queued.lock check at top of :CheckFolder). Data and mqparDIA.xml are already ready.
