...
testRank <- function(log2LFQt,sel1,sel2,cvThr){
  ...
  if(sum(!is.na(d1))>1&sum(!is.na(d2))>1){
    ...
    hist(log2FCmedianFC)
    ttest.results = data.frame(Uniprot=rowName,Gene=data$Gene.names,Protein=data$Protein.names,logFCmedianGrp1,logFCmedianGrp2,RankTestPval=pValNA,Log2MedianChange=logFCmedian,Log2AverageChange=logFCaverage,dataSellog2grpRankTest,RowGeneUniProtScorePeps=rownames(dataSellog2grpRankTest))
