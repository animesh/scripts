...
```{r MctrlS, echo = FALSE}
inpD<-"L:/promec/Animesh/Camilla/"
inpF<-"proteinGroups.txt"
data <- read.delim(paste0(inpD,inpF),sep="\t",header = T)#,row.names = 1)
#clean
...
Uniprot<-sapply(strsplit(protein,";"), `[`, 1)
...
dim(LFQ)
hist(log2LFQ)
colnames(log2LFQ)[c(seq(1,7,2),8,11,12,14,18,20,21,24,25)]
hda<-as.matrix(log2LFQ[,c(seq(1,7,2),8,11,12,14,18,20,21,24,25)])
hist(hda)
colnames(hda)
#ctr2<-apply(hda,1,function(x) if(sum(is.na(x[c(2,3,5)]))<3){median(x[c(2,3,5)],na.rm=T)} else{0})
MB4<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<3){median(x[c(1:3)],na.rm=T)} else{0})
#MB4<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<1){median(x[c(1:3)],na.rm=T)} else{0})
sum(MB4>0)
MCCAR<-apply(hda,1,function(x) if(sum(is.na(x[c(4:8)]))<5){median(x[c(4:8)],na.rm=T)} else{0})#2906 matrix20 session9 
#MCCAR<-apply(hda,1,function(x) if(sum(is.na(x[c(4:8)]))<1){median(x[c(4:8)],na.rm=T)} else{0})
sum(MCCAR>0)
MONO<-apply(hda,1,function(x) if(sum(is.na(x[c(9:13)]))<5){median(x[c(9:13)],na.rm=T)} else{0})
#MONO<-apply(hda,1,function(x) if(sum(is.na(x[c(9:13)]))<1){median(x[c(9:13)],na.rm=T)} else{0})
sum(MONO>0)
MctrlS=cbind(MB4,MCCAR,MONO)
summary(MctrlS)
inpW<-paste0(outD,gsub("[^[:alnum:] ]", "",inpD),inpF,"MctrlS.xlsx")
write_xlsx(as.data.frame(cbind(fasta,protein,gene,unipro,hda,MctrlS)),inpW)
write.table(as.data.frame(cbind(fasta,protein,gene,unipro,hda,MctrlS)),paste0(inpW,".txt"),sep="\t",quote=T,row.names = F)
MctrlS<-as.data.frame(MctrlS)
library("limma")
vennDiagram(MctrlS>0)
row.names(MctrlS)<-protein
MctrlS<-cbind(MctrlS,row.names(MctrlS),gen)
#hdaM<-cbind(hdaM,unipro)
summary(MctrlS)
```

```{r JJN3, echo = FALSE}
inpD2 <-"L:/promec/USERS/Voin/2016-07-12_QE_12samples/txt/"
...
hist(log2LFQ)
library("writexl")
colnames(log2LFQ)[c(1,5,9)]
hda<-as.matrix(log2LFQ[,c(1,5,9)])
hist(hda)
colnames(hda)
JJN3<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<3){median(x[c(1:3)],na.rm=T)} else{0})
#JJN3<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<1){median(x[c(1:3)],na.rm=T)} else{0})#4974 matrix85 session9 
sum(JJN3>0)
summary(hda)
hdaM=cbind(JJN3)
summary(hdaM)
inpW<-paste0(outD,gsub("[^[:alnum:] ]", "",inpD2),inpF2,"JJN3.xlsx")
write_xlsx(as.data.frame(cbind(fasta,protein,gene,unipro,hda,hdaM)),inpW)
write.table(as.data.frame(cbind(fasta,protein,gene,unipro,hda,hdaM)),paste0(inpW,".txt"),sep="\t",quote=T,row.names=F)
library("limma")
vennDiagram(hdaM>0)
JJN3<-as.data.frame(JJN3)
row.names(JJN3)<-protein
JJN3<-cbind(JJN3,row.names(JJN3),gen)
...
dataMid<-"gen"
ncol(JJN3)
dataM<-merge(JJN3,MctrlS,by.x=dataMid,by.y=dataMid,all=TRUE)
summary(dataM)
inpW<-paste0(outD,gsub("[^[:alnum:] ]", "",inpD),inpF,gsub("[^[:alnum:] ]", "",inpD2),inpF2,dataMid)
write_xlsx(as.data.frame(cbind(dataM,row.names(dataM))),paste0(inpW,".xlsx"))
write.table(as.data.frame(cbind(dataM,row.names(dataM))),paste0(inpW,".txt"),sep="\t",quote=T,row.names=F)
protNum<-1:nrow(dataM)
rn<-paste(protNum,dataM$`row.names(JJN3)`,dataM$`row.names(MctrlS)`,protNum,sep=";")
gn<-dataM$gen
#dataM<-dataM[,c(2:ncol(dataM))]
dataM<-dataM[,c(2:(ncol(JJN3)-1),(ncol(JJN3)+1):(ncol(dataM)-1))]#gene
row.names(dataM)<-rn
#install.packages("BiocManager")
#BiocManager::install("limma")
summary(dataM)
hist(as.matrix(dataM))
library(limma)
vennDiagram(dataM>0)
dataM<-cbind(dataM,gen)
```

```{r data2-merge-select}
thrD<-0
#dataS<-dataM[(dataM$JJN3>thrD)&(dataM$MB4>thrD)&(dataM$MCCAR>thrD)&(dataM$MONO==thrD|is.na(dataM$MONO)),2:5]
#dataS<-dataM[(dataM$JJN3==thrD)|(dataM$MB4==thrD)|(dataM$MCCAR==thrD)|(dataM$MONO==thrD),]
dataS<-dataM
dataS[is.na(dataS)]<-0
dataS<-dataS[(dataS$JJN3>thrD)|(dataS$MB4>thrD)|(dataS$MCCAR>thrD)|(dataS$MONO>thrD),]
dataS[dataS==0]<-NA
hist(rowMeans(as.matrix(dataS)))
plot(dataS)
vennDiagram(dataS>thrD)
```

```{r TK131416, echo = FALSE}
inpD4 <- outD
inpF4<-"proteinGroups.txtsel-LFQ-onevar-x1-control-sensel.txt"
data <- read.delim(paste0(inpD4,inpF4),sep="\t",header = T)#,row.names = 1)
protNum<-1:nrow(data)
fasta<-paste(protNum,data$fasta,protNum,sep=";")
gene<-toupper(as.character(sapply(strsplit(fasta,"_"), `[`, 1)))
gen<-as.character(sapply(strsplit(gene,"\\|"), `[`, 3))
...
hist(log2LFQ)
library("writexl")
colnames(log2LFQ)[c(2:4)]
hda<-as.matrix(log2LFQ[,c(2:4)])
hist(hda)
colnames(hda)
TK131416<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<1){median(x[c(1:3)],na.rm=T)} else{0})
#JJN3<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<1){median(x[c(1:3)],na.rm=T)} else{0})#4974 matrix85 session9 
sum(TK131416>0)
summary(TK131416)
hdaM=cbind(TK131416)
summary(hdaM)
inpW<-paste0(outD,gsub("[^[:alnum:] ]", "",inpD4),inpF4,"TK131416.xlsx")
write_xlsx(as.data.frame(cbind(fasta,protein,gene,unipro,hda,hdaM)),inpW)
write.table(as.data.frame(cbind(fasta,protein,gene,unipro,hda,hdaM)),paste0(inpW,".txt"),sep="\t",quote=T,row.names=F)
library("limma")
hist(hdaM)
vennDiagram(hdaM>0)
TK131416<-as.data.frame(TK131416)
row.names(TK131416)<-paste(unipro,protein,sep=";")
TK131416<-cbind(TK131416,row.names(TK131416),gen)
#TK131416<-cbind(TK131416,unipro)
summary(TK131416)
```
...
colnames(TK131416)
colnames(MctrlS)
dataM<-merge(JJN3,MctrlS,by.x=dataMid,by.y=dataMid,all=T)
summary(dataM)
library("limma")
colnames(dataM)
hist(as.matrix(dataM[,c(2,4:6)]))
vennDiagram(dataM[,c(2,4:6)]>0)
dataM<-merge(TK131416,dataM,by.x=dataMid,by.y=dataMid,all=T)
summary(dataM)
library("limma")
colnames(dataM)
hist(as.matrix(dataM[,c(2,4,6:8)]))
vennDiagram(dataM[,c(2,4,6:8)]>0)
inpW<-paste0(outD,gsub("[^[:alnum:] ]", "",inpD),inpF,gsub("[^[:alnum:] ]", "",inpD2),inpF2,dataMid)
write_xlsx(as.data.frame(cbind(dataM,row.names(dataM))),paste0(inpW,".xlsx"))
write.table(as.data.frame(cbind(dataM,row.names(dataM))),paste0(inpW,".txt"),sep="\t",quote=T,row.names=F)
protNum<-1:nrow(dataM)
...
```{r data2-merge-allIDs}
#https://stackoverflow.com/a/4882807/1137129
...
combD<-combineByName(t(JJN3),t(hdaM))
```

```{r sensel}
dataS<-read_xlsx(inpW)
dataS<-dataS[(dataS$ctr2==0) & (dataS$ctr3>0) & (dataS$ctr4>0) & (dataS$ctr6>0),]
...
#dataS<-dataSezWP
dataSezWPtest<-dataSezWP[,c(15,16,17,18)]
cn<-strsplit(colnames(dataSezWPtest), "_")
colnames(dataSezWPtest)<-paste(sapply(cn, "[", 2),sapply(cn, "[", 4))
vennDiagram(dataSezWPtest<selThr,main="dataSezWPtest",counts.col = "red")
```
...
chk<-dataSelApoptosis[dataSelApoptosis$`APIM4 2`>selThr&dataSelApoptosis$`APIM4 3`<selThr&dataSelApoptosis$`APIM4 4`<selThr&dataSelApoptosis$`APIM4 6`<selThr,]
write_xlsx(chk,paste0(inpF,"apoptosis.sensonly.xlsx"))
```
...
chk<-dataSel_PI3KAKTmTOR[dataSel_PI3KAKTmTOR$`APIM4 2`>selThr&dataSel_PI3KAKTmTOR$`APIM4 3`<selThr&dataSel_PI3KAKTmTOR$`APIM4 4`<selThr&dataSel_PI3KAKTmTOR$`APIM4 6`<selThr,]
write_xlsx(chk,paste0(inpF,"PI3KAKTmTOR.sensonly.xlsx"))
```
