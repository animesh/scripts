...
rownames(data)<-paste0(data$T..T..Uniprot,data$T..T..Line,data$T..T..Gene.names)
summary(data)
```
...
```{r sel-data-rows, echo = FALSE}
dataSel <- dataS[grep("stat", data$C..Gene.ontology..GO.,ignore.case=TRUE),] 
#dataSel<-data[,grep("_x.y_",colnames(data))]
...
```{r sel-data-rows, echo = FALSE}
dataSel <- dataS[grep("mapk", data$C..Gene.ontology..GO.,ignore.case=TRUE),] 
#dataSel<-data[,grep("_x.y_",colnames(data))]
dataSel<-dataSel[,grep("_x.y_",colnames(dataSel))]
dataSel<-dataSel[,-grep("D_x.y_",colnames(dataSel))]
dataSel<-dataSel[,-grep("C_x.y_",colnames(dataSel))]
...
```{r sel-data-rows, echo = FALSE}
data_apop <- dataS[grep("apopto", data$C..Gene.ontology..GO.,ignore.case=TRUE),] 
#dataSel<-data[,grep("_x.y_",colnames(data))]
dataSel<-data_apop[,grep("_x.y_",colnames(data_apop))]
dataSel<-dataSel[,-grep("D_x.y_",colnames(dataSel))]
dataSel<-dataSel[,-grep("C_x.y_",colnames(dataSel))]
#data_apop <- data[grep("apoptosis", data$C..Gene.ontology..GO.),] 
#data_jak <- data[grep("apoptosis", data$C..Gene.ontology..GO.),] 
...
data_selr<-rbind.data.frame(dataSelApoptosis,dataSelMAPK,dataSelSTAT)
#data_selr<-dataSel
...
svgPHC<-pheatmap(data_selr,scale="row",clustering_distance_rows = "correlation",clustering_distance_cols = "correlation",fontsize_row=6,cluster_cols=FALSE,,cluster_rows=FALSE)#,annotation_col = label,show_rownames=F)
library(ggplot2)
#ggsave(file=paste0(inpF,"corrcoefED.svg"), plot=svgPHC, width=6, height=6)
#plot(svgPHC)
ggsave(file=paste0(inpD,"clusterPlot.ApoptoMAPKSTAT.svg"),plot=svgPHC)#,  width=6, height=6)
