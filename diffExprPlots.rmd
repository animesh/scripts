...
library("readxl")
library("writexl")
inpW<-paste0(inpF,selection,".log2.xlsx")
write_xlsx(as.data.frame(cbind(fasta,protein,gene,log2LFQ,unipro)),inpW)
#write_xlsx(as.data.frame(data),inpW)
...
sum(MB4>0)
#MCCAR<-apply(hda,1,function(x) if(sum(is.na(x[c(4:8)]))<5){median(x[c(4:8)],na.rm=T)} else{0})
...
summary(hdaM)
inpW<-paste0(inpF,"sel-LFQ-onevar-control.xlsx")
...
```{r data2, echo = FALSE}
inpD <-"L:/promec/Animesh/Voin/2016-08-26_12-T-samples/"
inpF<-paste0(inpD,"proteinGroups.txt")
data <- read.delim(inpF,sep="\t",header = T)#,row.names = 1)
data = data[!data$Reverse=="+",]
#data = data[!data$C..Potential.contaminant=="+",]
...
inpW<-paste0(inpF,selection,".log2.xlsx")
write_xlsx(as.data.frame(cbind(fasta,protein,gene,log2LFQ,unipro)),inpW)
colnames(log2LFQ)[c(1:3)]
hda<-as.matrix(log2LFQ[,c(1:3)])
hist(hda)
colnames(hda)
#ctr2<-apply(hda,1,function(x) if(sum(is.na(x[c(2,3,5)]))<3){median(x[c(2,3,5)],na.rm=T)} else{0})
#JJN3<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<3){median(x[c(1:3)],na.rm=T)} else{0})
JJN3<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))<1){median(x[c(1:3)],na.rm=T)} else{0})
sum(JJN3>0)
summary(hda)
hdaM=cbind(JJN3)
summary(hdaM)
...
```{r data2-merge}
dataM<-merge(JJN3,hdaM,by.x="gen",by.y="gen",all=TRUE)
summary(dataM)
inpW<-paste0(inpF,"sel-LFQ-control-combined.xlsx")
write_xlsx(as.data.frame(cbind(dataM,row.names(dataM))),inpW)
inpW<-paste0(inpF,"sel-LFQ-control-comb.xlsx")
write.csv(as.data.frame(dataM),paste0(inpW,".csv"))
write_xlsx(as.data.frame(dataM),inpW)
protNum<-1:nrow(dataM)
row.names(dataM)<-paste(protNum,dataM$unipro,protNum,sep=";")
dataM<-dataM[,2:ncol(dataM)]
