...
```{r data, echo = FALSE}
inpD1 <-"L:/promec/Animesh/Camilla/"
...
sum(MONO>0)
hdaM=cbind(MB4,MCCAR,MONO)
summary(hdaM)
inpW<-paste0(outD,gsub("[^[:alnum:] ]", "",inpD),inpF,"sel-LFQ-onevar-control.xlsx")
...
hdaM<-as.data.frame(hdaM)
library("limma")
vennDiagram(hdaM>0)
row.names(hdaM)<-protein
#hdaM<-cbind(hdaM,row.names(hdaM),gen)
...
hdaM=cbind(JJN3)
summary(hdaM)
inpW<-paste0(outD,gsub("[^[:alnum:] ]", "",inpD2),inpF2,"sel-LFQ-onevar-control.xlsx")
write_xlsx(as.data.frame(cbind(fasta,protein,gene,unipro,hda,hdaM)),inpW)
write.table(as.data.frame(cbind(fasta,protein,gene,unipro,hda,hdaM)),paste0(inpW,".txt"),sep="\t",quote=T,row.names=F)
library("limma")
vennDiagram(hdaM>0)
...
#dataMid<-"gen"
ncol(JJN3)
dataM<-merge(JJN3,hdaM,by.x=dataMid,by.y=dataMid,all=TRUE)
summary(dataM)
...
row.names(dataM)<-paste(protNum,dataM$`row.names(JJN3)`,dataM$`row.names(hdaM)`,protNum,sep=";")
