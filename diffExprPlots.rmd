```{r Pasientceller}
library(readxl)
inpD<-"L:/promec/Animesh/Sigrid/Hierarchical clustering/"
inpW<-paste0(inpD,"Pasientceller mut vs wt uten beh til publ.xlsx")
data<-read_xlsx(inpW)
data<-data.frame(data)
dataS<-data[,grep("Slot1",colnames(data))]
dataS<-sapply(dataS,as.numeric)
hist(dataS)
dataPasientceller<-data.frame(cbind(RowGeneUniProtScorePeps=data[,"RowGeneUniProtScorePeps"],dataS))
rownames(dataS)<-data[,"Gene"]
data_selr_na0=(dataS-apply(dataS,1,function(x) median(x,na.rm=T)))/apply(dataS,1,function(x) sd(x,na.rm=T))
#scales::squish(as.matrix(dataS),c(-10,10))
data_selr_na0[is.na(data_selr_na0)]=0
hist(data_selr_na0)
data_selr_na0=data_selr_na0[rowSums(data_selr_na0)!=0,]
svgPHC=pheatmap::pheatmap(data_selr_na0,clustering_distance_rows="correlation",clustering_distance_cols = "correlation",cluster_cols=T,cluster_rows=T,fontsize_col=8,fontsize_row=1)
ggplot2::ggsave(file=paste0(inpW,"clusterPlot.svg"),plot=svgPHC,dpi=320,width=6,height=12)#,  
write.csv(cbind(rownames(data_selr_na0),data_selr_na0),paste0(inpW,".data_selr.csv"),row.names = F)
...
library(readxl)
inpD<-"L:/promec/Animesh/Sigrid/Hierarchical clustering/"
inpW<-paste0(inpD,"U87MG mut vs wt uten beh til publ.xlsx")
data<-read_xlsx(inpW,skip=1)
dataS<-data[,grep("Slot2",colnames(data))]
dataS<-sapply(dataS,as.numeric)
hist(dataS)
dataU87MG<-data.frame(cbind(RowGeneUniProtScorePeps=data[,"ID"],dataS))
#rownames(dataS)<-data[,"ID"]#data[,"MUTCtrlWTCtrlGene"],data[,"MUTCtrlWTCtrlUniprot"],sep=";")
data_selr_na0=(dataS-apply(dataS,1,function(x) median(x,na.rm=T)))/apply(dataS,1,function(x) sd(x,na.rm=T))
#scales::squish(as.matrix(dataS),c(-10,10))
data_selr_na0[is.na(data_selr_na0)]=0
hist(data_selr_na0)
...
```{r merge}
inpD<-"L:/promec/Animesh/Sigrid/Hierarchical clustering/"
inpW<-paste0(inpD,"mergePasientcellerU87MG.xlsx")
dataU87MG$Gene=paste(sapply(strsplit(paste(sapply(strsplit(dataU87MG$ID, ";;",fixed=T), "[", 5)), ";"), "[", 1))
dataPasientceller$Gene=paste(sapply(strsplit(paste(sapply(strsplit(dataPasientceller[,"RowGeneUniProtScorePeps"], ";;",fixed=T), "[", 5)), ";"), "[", 1))
dataMerge<-merge(dataPasientceller,dataU87MG,by="Gene",all=T)
#rownames(dataS)<-data[,"ID"]#data[,"MUTCtrlWTCtrlGene"],data[,"MUTCtrlWTCtrlUniprot"],sep=";")
rN<-paste(dataMerge$Gene,1:nrow(dataMerge),sep=";")
dataMerge=dataMerge[,grep("Slot",colnames(dataMerge))]
dataMerge=sapply(dataMerge,as.numeric)
rownames(dataMerge)=rN
hist(dataMerge)
rowSums(dataMerge,na.rm = T)
dataMerge=dataMerge[complete.cases(dataMerge),]
#dataS=apply(dataMerge,2,function(x) rank(x))
dataS=(dataMerge-apply(dataMerge,2,function(x) median(x,na.rm=T)))/apply(dataMerge,2,function(x) sd(x,na.rm=T))
hist(dataS)
data_selr_na0=(dataS-apply(dataS,1,function(x) median(x,na.rm=T)))/apply(dataS,1,function(x) sd(x,na.rm=T))
#rownames(data_selr_na0)=rownames(dataMerge)
#scales::squish(as.matrix(dataS),c(-10,10))
data_selr_na0[is.na(data_selr_na0)]=0
hist(data_selr_na0)
data_selr_na0=data_selr_na0[rowSums(data_selr_na0)!=0,]
...
ggbetweenstats(data,Class,LAMP1.P11279.P11279.2.7)
ggbetweenstats(data,Class,RSU1.Q15404.Q15404.2.494)
pairs(trisl, 1:4], panel=function(X,y){
...
```{r data-apim-z-score}
inpD<-"F:/OneDrive - NTNU/Desktop/TK/"
...
```{r data-apim-z-score-ribosome}
dataRibo<-data[grep(";ribosome;",data$C..GOCC.name,fixed = T),]
data_selr<-dataRibo[,grep("_APIM",names(dataRibo))]
rn<-strsplit(dataRibo$T..Gene.names, ";",fixed=T)
rn2<-strsplit(dataRibo$T..Protein.IDs, ";",fixed=T)
...
ggplot2::ggsave(file=paste0(inpF,"clusterPlotAPIMdataRibo.svg"),plot=svgPHC,dpi=320,width=6,height=10)#,  
write.csv(cbind(rownames(data_selr),data_selr),paste0(inpF,".data_selr.APIM.dataRibo.csv"),row.names = F)
```

```{r data-apim-z-score-ATF}
dataATF<-data[grep("ATF",data$T..Gene.names,fixed = T),]
data_selr<-dataATF[,grep("_APIM",names(dataATF))]
rn<-strsplit(dataATF$T..Gene.names, ";",fixed=T)
rn2<-strsplit(dataATF$T..Protein.IDs, ";",fixed=T)
row.names(data_selr)<-paste(sapply(rn, "[", 1),sapply(rn2, "[", 1),sep=";")#,sapply(rn, "[", 6))
cn<-strsplit(colnames(data_selr), "_")
colnames(data_selr)<-paste(sapply(cn, "[", 1))
data_selr_na0<-data_selr
...
hist(data_selr_na0)
svgPHC=pheatmap::pheatmap(data_selr_na0,clustering_distance_rows="euclidean",clustering_distance_cols = "euclidean",cluster_cols=F,cluster_rows=F,fontsize_col=8,fontsize_row=5)
ggplot2::ggsave(file=paste0(inpF,"clusterPlotAPIMdataATF.svg"),plot=svgPHC)#,  
write.csv(cbind(rownames(data_selr),data_selr),paste0(inpF,".data_selr.APIM.dataATF.csv"),row.names = F)
```

```{r data-apim-z-score-EIF2AK}
dataGNC2<-data[grep("EIF2AK",data$T..Gene.names,fixed = T),]#EIF2AK4 is not in the list?
data_selr<-dataGNC2[,grep("_APIM",names(dataGNC2))]
rn<-strsplit(dataGNC2$T..Gene.names, ";",fixed=T)
rn2<-strsplit(dataGNC2$T..Protein.IDs, ";",fixed=T)
row.names(data_selr)<-paste(sapply(rn, "[", 1),sapply(rn2, "[", 1),sep=";")#,sapply(rn, "[", 6))
cn<-strsplit(colnames(data_selr), "_")
colnames(data_selr)<-paste(sapply(cn, "[", 1))
data_selr_na0<-data_selr
...
```{r data-ctr-z-score}
inpD<-"L:/promec/Animesh/Jana/reproOlaisen/"
...
p<-pheatmap::pheatmap(data_selr_na0,clustering_distance_rows=dsubCor,clustering_distance_cols = "binary",cluster_cols=F,cluster_rows=T,fontsize_col=8,fontsize_row=4)
