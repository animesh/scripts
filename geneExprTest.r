#..\R-4.5.0\bin\Rscript.exe geneExprTest.r "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/report.gg_matrix.tsv" "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/GroupsXtreme.txt" "Groups" "Rem" 1 4 4 "Good responders" "Poor responders" 0.1 0.5 0.1
#..\R-4.5.0\bin\Rscript.exe geneExprTest.r "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/report.gg_matrix.tsv" "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/Groups45GyXtreme.txt" "Groups" "Rem" 1 4 4 "Good responders" "Poor responders" 0.1 0.5 0.1
#..\R-4.5.0\bin\Rscript.exe geneExprTest.r "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/report.gg_matrix.tsv" "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/Groups60GyXtreme.txt" "Groups" "Rem" 1 4 4 "Good responders" "Poor responders" 0.1 0.5 0.1
#..\R-4.5.0\bin\Rscript.exe geneExprTest.r "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/report.gg_matrix.tsv" "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/GroupsXtremeBigC.txt" "Groups" "Rem" 1 4 4 "Good responders" "Poor responders" 0.1 0.5 0.1
#..\R-4.5.0\bin\Rscript.exe geneExprTest.r "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/report.gg_matrix.tsv" "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/Groups45GyXtremeBigC.txt" "Groups" "Rem" 1 4 4 "Good responders" "Poor responders" 0.1 0.5 0.1
#..\R-4.5.0\bin\Rscript.exe geneExprTest.r "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/report.gg_matrix.tsv" "L:/promec/TIMSTOF/LARS/2025/250902_Alessandro/DIANNv2p2/Groups60GyXtremeBigC.txt" "Groups" "Rem" 1 4 4 "Good responders" "Poor responders" 0.1 0.5 0.1
#setup####
...
row.names(data)<-paste(row.names(data),data[,grep("Gene",colnames(data))],data[,grep("Proteotypic",colnames(data))],sep=";;")
summary(data)
dim(data)
##int####
...
testT <- function(log2LFQ,sel1,sel2,cvThr,dfName){
  ...
  intdataCVd1<-2^d1
  cvIntD1<-(apply(intdataCVd1,1,function(x) sd(x,na.rm=T)/mean(x,na.rm=T)))*100
  hist(cvIntD1,main = paste("Gene Group Intensity",sel1,sep="\n"),sub=paste(list(colnames(intdataCVd1)),sep="\n"),font.sub=3,xlab = "%CV",ylab="Frequency",breaks=length(cvIntD1))
  d2<-data.frame(log2LFQ[,gsub("-",".",rownames(label[label$pair2test==sel2,]))])
  colnames(d2)<-gsub("-",".",rownames(label[label$pair2test==sel2,]))
  intdataCVd2<-2^d2
  cvIntD2<-(apply(intdataCVd2,1,function(x) sd(x,na.rm=T)/mean(x,na.rm=T)))*100
  hist(cvIntD2,main = paste("Gene Group Intensity",sel2,sep="\n"),sub=paste(list(colnames(intdataCVd2)),sep="\n"),font.sub=3,xlab = "%CV",ylab="Frequency",breaks=length(cvIntD2))
  dataSellog2grpTtest<-merge(d1, d2, by = 'row.names', all = TRUE)
  rN<-dataSellog2grpTtest[,1]
  geneName<-paste(sapply(strsplit(paste(sapply(strsplit(rN, ";;",fixed=T), "[", 2)), "[; ]"), "[", 1))
  proteotypicPeptides<-as.numeric(sapply(strsplit(paste(sapply(strsplit(rN, ";;",fixed=F), "[", 3)), "\\|"), "[", 1))
  hist(proteotypicPeptides,main = paste("GproteotypicPeptides",dfName,sep="\n"),sub=paste(list(colnames(dataSellog2grpTtest)),sep="\n"),font.sub=3,xlab = "#peptides",ylab="Frequency",breaks=length(proteotypicPeptides))
  geneName[is.na(geneName)]=rN[is.na(geneName)]
  ...
  write.table(cbind(gene_id=gene_id,geneName,proteotypicPeptides,rN),paste0(inpD,"/",sel1,sel2,dfName,"annotation.tsv"),row.names = F,quote = F,sep="\t")
  ...
  if(sum(!is.na(d1))>1&sum(!is.na(d2))>1){
    ...
    if(sum(is.na(pValNA))<length(pValNA)){
      ...
      hist(grp1CV,main = paste("Coefficient of Variation",sel1,sep="\n"),sub=paste(list(colnames(d1)),sep="\n"),font.sub=3,xlab = "CV",ylab="Frequency",breaks=length(grp1CV))
      ...
      hist(grp2CV,main = paste("Coefficient of Variation",sel2,sep="\n"),sub=paste(list(colnames(d2)),sep="\n"),font.sub=3,xlab = "CV",ylab="Frequency",breaks=length(grp2CV))
      ...
      hist(log2FCmedianFC)
      ttest.results = data.frame(Gene=geneName,Peptides=proteotypicPeptides,logFCmedianGrp1,logFCmedianGrp2,PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,TtestPval=pValNA,dataSellog2grpTtest,Log2MedianChange=logFCmedian,grp1CV,grp2CV,RowGeneUniProtScorePeps=rN)
