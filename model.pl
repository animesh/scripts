    ...
    if($codon eq $stop_codon)
    {
      ...
      while($stack1_top!=-1)
      {
        ...
        if($difference%3==0 && $difference>=60)
        {
          ...
          for(my $ctr1=0;$ctr1<$noe;$ctr1+=2)
          {
            if($gene[$ctr1+1]==$i+3)
            {
             #print "  A:$gene[$ctr1+1]       B:$i   ";
             $added_flag=1;
             if($start_index<$gene[$ctr1])
             {
           	#print "A correction was made";
           	print "** Start Index : $start_index Gene: $gene[$ctr1] $ctr1\n";
           	$gene[$ctr1]=$start_index;
                print "** Start Index : $start_index Gene: $gene[$ctr1] $ctr1\n";
              }
            }  	
          }
