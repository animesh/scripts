...
if ($#ARGV < 0)
{
    ...
    print "  of chains, filter the list to pick out the good alignments\n";
...
for ($i = 0; $i < $num_files; $i++)
{
    ...
    {
	if ($dataline =~ /^chain (\d+) .+ \d+ . \d+ \d+ (.+) \d+ . \d+ \d+/)
	{
	    $score = $1;
	    $read_id = $2;

	    push @chains, "$read_id $score == $dataline";
	}
    }
