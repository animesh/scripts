...
#!/usr/bin/perl
#final CDS extractor based on the chi stat
$file=shift @ARGV;
$file2=shift @ARGV;
$pk=shift @ARGV;
$choose= shift @ARGV;
...
$st=@t1[3]+0;$sp=@t1[4]+0;$length=$sp-$st+1;
$str = uc(substr($line,($st-1),($length)));
$t11=@t1[3];#$t11=~s/\>|\s+//g;
if($t11 =~ /^c/){
@t1[3]=~s/c//;
$st=@t1[4]+0;$sp=@t1[3]+0;$length=$sp-$st+1;
$str = uc(substr($line,($st-1),($length)));
#$st=@t1[2]-@t1[9]+1-3;$sp=@t1[2]-@t1[6]+1;$length=$sp-$st+1;$str = uc(substr($line,($st-1),($length)));
$str = reverse ($str);
$str =~ tr/ATCG/TAGC/d;chomp $str;
if($str=~/^(TTG|GTG|ATG)/ and $str=~/(TAA|TAG|TGA)$/ and ($length >= 453) and ($length <= 1233)){
