...
row.names(data)<-paste(row.names(data),apply(data[, seq(proteinC,as.numeric(selection)-1,1)], 1, paste, collapse = ";;"),sep=";;")
summary(data)
dim(data)
##int####
...
testT <- function(log2LFQ,sel1,sel2,cvThr,dfName){
  ...
  #colnames(log2LFQ)
  #dfName="LFQvsn"#"log2LFQselMMdata"#"log2LFQsel"
  ...
  geneName<-paste(sapply(strsplit(paste(sapply(strsplit(rN, ";;",fixed=T), "[", 4)), "[; ]"), "[", 1))
  proteotypicPeptides<-as.numeric(sapply(strsplit(paste(sapply(strsplit(rN, ";;",fixed=F), "[", 7)), "\\|"), "[", 1))
