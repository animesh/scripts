...
hist(hda)
pathview(hda,pathway.id=pcode,species=pspecies,gene.idtype=idtype,low=list(gene="blue"),mid=list(gene="white"),high=list(gene="orange"),both.dirs=list(gene=T), limit = list(gene = 10), bins = list(gene = 20), na.col="grey",out=sfx)
write_xlsx(data.frame(Name=data$RowGeneUniProtScorePeps,hda),paste0(inpF,sfx,"wTestBHgrpComb.xlsx"))
```

```{r pathview-comb-AP[ET]/pc}
#https://bioconductor.org/packages/release/bioc/vignettes/pathview/inst/doc/pathview.pdf
#BiocManager::install("pathview")
library(pathview)
library(writexl)
