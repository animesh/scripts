...
my $ucnt;
my $lcnt;
my $of1=$fileflow.$file.".train.csv";
open(FO1,">$of1");
my $of2=$fileflow.$file.".trainname.csv";
open(FO2,">$of2");
my $of3=$fileflow.$file.".test.csv";
open(FO3,">$of3");
my $of4=$fileflow.$file.".testname.csv";
open(FO4,">$of4");
...
#	if($hitscore{$_}<=$lowr && $lcnt<$seqcntthresh){print FO1"$flowval{$_}B\n";$lcnt++;}
	...
	elsif($hitscore{$_}>(($max-$min)/2)&&($hitscore{$_}<$upr)){print FO3"$flowval{$_}T\n";$ucnt++;print FO4"$_\n"}
	elsif($hitscore{$_}<=(($max-$min)/2)&&($hitscore{$_}>$lowr)){print FO3"$flowval{$_}B\n";$lcnt++;print FO4"$_\n"}
}
