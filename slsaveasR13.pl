...
# $Revision $
 ...
 chomp($renamedBlkParams = @all_lines[10]);
 ...
 #Create the renamed params map. It is keyed by blockType and has a submap with
 #currParam => oldParam pairs
 %renamedParamsHash = ();
 foreach $entry (split(/\|\|/, $renamedBlkParams)) {
     ($bType, $params) = split(/\|/, $entry, 2);
     %paramsHash = (split(/\|/, $params));
     $renamedParamsHash{$bType} = {%paramsHash};
 }
...
sub UpdateModel
{
  ...
  while (<IN>) {  
   ...
   if($mdlFlag || $libFlag){
     ...
     # Special case hack for SolverName param
     if(/\s*SolverName\s*/) {
	 s/SolverName/Solver/g;
     }
   }
   ...
   if ($blkParamDefaultsFlag) {
	   ...
	   $dontPrint = SkipBlockParam($line, $blkType, $verString);
	   ...
	   # Rename block params
	   ReplaceBlockParam($line, $blkType);
       }
   ...
   # The BlockParameterDefaults section ends when we find
   # either AnnotationDefaults or System.
   if ($blkParamDefaultsFlag == 1) {
...
# Function: ReplaceBlockParam ==================================================
sub ReplaceBlockParam{
    
    my($line, $blkType) = @_;
    
    ($param, $value) = $line =~ /\s*(\w+)\s*[\"|\{|\[]*.+/;

    # Check if this param is in the renamedParamsHash. If it is, we send 
    # back a modified version of the line. Otherwise, we send back the original line
    $oldParam = $renamedParamsHash{$blkType}{$param};
    if ($oldParam ne "") {
	$line = $_;
	s/$param/$oldParam/g;
    }
...
sub SkipBlockParam{
  ...
  # Special case: Handle SampleTime parameter
  # If we are saving to R13 FCS, do not remove sample time info for these blocks:
  # Unit Delay 
  # Discrete Filter 
  # Digital Clock 
  ...
  # Zero-Order Hold 
  if ($verString eq "5.0" && $param eq "SampleTime") {
      $retVal = 1; # assume
      @stBlkList = ("UnitDelay", "DiscreteFilter", "DiscreteZeroPole", "DiscreteIntegrator", 
                    "DiscreteStateSpace", "DiscreteTransferFcn", "ZeroOrderHold", "Inport",
                    "Sin", "FromFile", "FromWorkspace", "Step", "RandomNumber", 
                    "UniformRandomNumber", "DiscretePulseGenerator", "DigitalClock", "ToFile",
                    "ToWorkspace", "Scope", "Display");
      foreach $item (@stBlkList) {
          if ($item eq $blkType) {
              $retVal = 0;
              last;
          }
