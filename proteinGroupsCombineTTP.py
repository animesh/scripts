...
#pathFiles=Path("C:/Users/animeshs/Desktop/dda/FP")
fileName='*quant.csv'#'protein_label_quant.tsv'
trainList=list(pathFiles.rglob(fileName))
#!pip3 install pandas --user
import pandas as pd
#df = pd.concat(map(pd.read_table, trainList))
#df.to_csv(pathFiles.with_suffix('.combinedT.txt'),sep="\t")#,rownames=FALSE)
#f=trainList[0]#Path("C:/Users/animeshs/Desktop/FP2/210902_sudhl5_tot_3_Slot1-30_1_181/protein_label_quant.tsv")
df=pd.DataFrame()
for f in trainList:
    if Path(f).stat().st_size > 0:
        ...
        proteinHits['ID']=proteinHits['modified_peptide']+(proteinHits['charge']).astype(str)
        proteinHits.rename({'intensity':'MedianLog2SILAC'},inplace=True,axis='columns')
...
df=df.pivot_table(index='ID', columns='Name', values='MedianLog2SILAC', aggfunc='sum')
