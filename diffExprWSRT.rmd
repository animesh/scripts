...
#install.packages("readxl")
#library(readxl)
resMatlab<-readxl::read_excel("L:\\promec\\HF\\Lars\\2020\\AUGUST\\siri\\combined\\txt_PHOSTY\\proteinGroups.txt WSRTpval.xls")
resMatlab<-as.numeric(resMatlab)
summary(resMatlab)
hist(as.matrix(resMatlab[,7]-dataSellog2grpWSRT[,1]))#,(resMatlab[,7]+dataSellog2grpWSRT[,1]))
hist(as.matrix(resMatlab[,3]))
hist(pValNA)
hist(as.matrix(resMatlab[,3])-pValNA)
