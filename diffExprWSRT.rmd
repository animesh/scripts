...
pValNA0 = apply(
  dataSellog2grpWSRT, 1, function(x)
    if(sum(!is.na(x))<2){NA}
    else if((max(x,na.rm=T)-min(x,na.rm=T))==0){0}
#    else if(sum(!is.na(x[c(sCol:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))>=2){t.test(as.numeric(x[c(sCol:mCol)]),as.numeric(x[c((mCol+1):eCol)]),na.rm=T,var.equal=F)$p.value}
...
pValNA = apply(
  dataSellog2grpWSRT, 1, function(x)
    if(sum(!is.na(x))<2){NA}
    else if(max(x,na.rm=T)==min(x,na.rm=T)){0}
#    else if(sum(!is.na(x[c(sCol:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))>=2){t.test(as.numeric(x[c(sCol:mCol)]),as.numeric(x[c((mCol+1):eCol)]),na.rm=T,var.equal=F)$p.value}
    else{wilcox.test(as.numeric(x),na.rm=T)$p.value}
  #else{NA}
  )
...
plot(pValNA,pValNA0)
plot(pValNA-pValNA0)
inpW<-paste0(inpF,"sel-LFQ-S-control.WSRT.xlsx")
writexl::write_xlsx(as.data.frame(cbind(uniprot,dataSellog2grpWSRT,pValNA,pValNA0,data[,7])),inpW)
```

```{r S_wsrtMatlab, echo = FALSE}
#install.packages("readxl")
#library(readxl)
resMatlab<-readxl::read_excel("L:\\promec\\HF\\Lars\\2020\\AUGUST\\siri\\combined\\txt_noPHO\\proteinGroupsSctr.xlsx WSRTpval.xls")
summary(resMatlab)
hist(as.matrix(resMatlab[,3]))
hist(as.matrix(resMatlab[,3])-pValNA0)
plot(as.matrix(resMatlab[,3]),pValNA)
plot(as.matrix(resMatlab[,3])-pValNA0)
```
