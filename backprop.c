...
{\
  ...
  register int _i,_l;\
  _to =index.html (char *)(to);\
  _from = (char *)(from);\
  _l = (len);\
  for (_i = 0; _i < _l; _i++) *_to++ =index.html *_from++;\
}
...
int m, n;
{
  ...
  for (i = 0; i < m; i++) {
    new[i] =index.html alloc_1d_dbl(n);
  }
...
int m, n;
{
  ...
  for (i = 0; i <=index.html m; i++) {
    for (j = 0; j <=index.html n; j++) {
      w[i][j] = dpn1();
    }
  }
}
...
int n1, n2;
{
  ...
  for (j = 1; j <=index.html n2; j++) {

    /*** Compute weighted sum of its inputs ***/
    sum = 0.0;
    for (k = 0; k <=index.html n1; k++) {
      sum += conn[k][j] * l1[k];
    }
    l2[j] = squash(sum);
  }
...
int nh, no;
{
  ...
  errsum = 0.0;
  for (j = 1; j <=index.html nh; j++) {
    h = hidden[j];
    sum = 0.0;
    for (k = 1; k <=index.html no; k++) {
...
double *delta, *ly, **w, **oldw, eta, momentum;
{
  ...
  ly[0] = 1.0;
  for (j = 1; j <=index.html ndelta; j++) {
    for (k = 0; k <=index.html nly; k++) {
