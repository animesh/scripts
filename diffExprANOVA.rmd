...
par(mfrow=c(1,2))
#library/packages
inpD<-"L:/promec/USERS/MarianneNymark/20200108_15-samples/QE/combined/txt/"
hdr<-gsub("[^[:alnum:] ]", "",inpD)
...
```{r WT}
hda<-as.matrix(log2LFQ[,grep("WT",colnames(log2LFQ))])
hist(as.matrix(hda))
...
```{r FTSY6}
hda<-as.matrix(log2LFQ[,grep("FTSY6",colnames(log2LFQ))])
hist(as.matrix(hda))
FTSY6<-apply(hda,1,function(x) if(sum(is.na(x))<ncol(hda)){median(x,na.rm=T)} else{0})
sum(FTSY6>0)
limma::vennDiagram(hda>0)
inpW<-paste0(inpF,"sel-LFQ-FTSY6.xlsx")
writexl::write_xlsx(as.data.frame(cbind(WT,hda,data$Fasta.headers)),inpW)
hdaFTSY6<-hda
```

```{r FTSY5}
hda<-as.matrix(log2LFQ[,grep("FTSY5",colnames(log2LFQ))])
hist(as.matrix(hda))
FTSY5<-apply(hda,1,function(x) if(sum(is.na(x))<ncol(hda)){median(x,na.rm=T)} else{0})
sum(FTSY5>0)
limma::vennDiagram(hda>0)
inpW<-paste0(inpF,"sel-LFQ-FTSY5.xlsx")
writexl::write_xlsx(as.data.frame(cbind(WT,hda,data$Fasta.headers)),inpW)
hdaFTSY5<-hda
limma::vennDiagram(hda>0)
```
...
```{r venn}
hda<-cbind(WT,FTSY5,FTSY6)
hist(hda)
limma::vennDiagram(hda>0)
```

```{r FTSY6WTt, echo = FALSE}
dataSellog2grpTtest<-cbind(hdaFTSY6,hdaWT)
...
writexl::write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,"wTestBHgrpFTSY6WT.xlsx"))
...
```{r FTSY5WTt, echo = FALSE}
dataSellog2grpTtest<-cbind(hdaFTSY5,hdaWT)
summary(dataSellog2grpTtest)
row.names(dataSellog2grpTtest)<-row.names(data)
sCol<-1
eCol<-10
...
hist(log2FCmedianFC)
ttest.results = data.frame(Uniprot=rowName,PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,TtestPval=pValNA,dataSellog2grpTtest,Log2MedianChange=logFCmedian,RowGeneUniProtScorePeps=rownames(dataSellog2grpTtest))
writexl::write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,"wTestBHgrpFTSY5WT.xlsx"))
#limma::vennDiagram(cbind(as.matrix(dataSellog2grpTtest[,c(sCol:mCol)]),as.matrix(dataSellog2grpTtest[,c((mCol+1):eCol)])))
limma::vennDiagram(cbind(logFCmedianGrp1,logFCmedianGrp2))
```

```{r FTSY6FTSY5t, echo = FALSE}
dataSellog2grpTtest<-cbind(hdaFTSY6,hdaFTSY5)
summary(dataSellog2grpTtest)
row.names(dataSellog2grpTtest)<-row.names(data)
sCol<-1
eCol<-10
...
hist(log2FCmedianFC)
ttest.results = data.frame(Uniprot=rowName,PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,TtestPval=pValNA,dataSellog2grpTtest,Log2MedianChange=logFCmedian,RowGeneUniProtScorePeps=rownames(dataSellog2grpTtest))
writexl::write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,"wTestBHgrpFTSY6FTSY5.xlsx"))
#limma::vennDiagram(cbind(as.matrix(dataSellog2grpTtest[,c(sCol:mCol)]),as.matrix(dataSellog2grpTtest[,c((mCol+1):eCol)])))
limma::vennDiagram(cbind(logFCmedianGrp1,logFCmedianGrp2))
```
...
hist(res$mean_WT)
hist(res$mean_FTSY6)
plot(res$mean_FTSY6,res$mean_WT)
boxplot(res$mean_FTSY6,res$mean_WT)
plotPCA(pepdat)
#plotVolcano_live(prodat.med, res)
```

```{r FTSY5WTp}
metadataFile <- file.path(paste0(inpD,"metadata.5.txt"))
meta <- read.delim(metadataFile, header=TRUE, sep="\t")
summary(meta)
summary(evi)
pepdat <- makePeptideTable(evi, meta)
...
hist(res$mean_WT)
hist(res$mean_FTSY5)
plot(res$mean_FTSY5,res$mean_WT)
boxplot(res$mean_FTSY5,res$mean_WT)
...
```{r FTSY6FTSY5p}
metadataFile <- file.path(paste0(inpD,"metadata.noWT.txt"))
meta <- read.delim(metadataFile, header=TRUE, sep="\t")
summary(meta)
summary(evi)
pepdat <- makePeptideTable(evi, meta)
...
hist(res$mean_FTSY6)
hist(res$mean_FTSY5)
plot(res$mean_FTSY5,res$mean_FTSY6)
boxplot(res$mean_FTSY5,res$mean_FTSY6)
#plotVolcano_live(prodat.med, res)
```
