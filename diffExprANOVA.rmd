...
par(mfrow=c(1,2))
#data
inpD <-"L:/promec/Qexactive/LARS/2021/march/Rolf/combined/txt/"
thr<-20
selThr=0.1
selThrFC=1
hdr<-gsub("[^[:alnum:] ]", "",inpD)
...
hda<-as.matrix(log2LFQ[,rownames(label[grep(sel,label$Diet),])])
summary(hda)
hist(as.matrix(hda))
#hda1<-apply(hda[,1:3],1,function(x) if(sum(is.na(x))<ncol(hda)){median(x,na.rm=T)} else{0})
sum(hda1>thr,na.rm=T)
hda1<-apply(hda[,1:2],1,function(x) if(sum(is.na(x))<ncol(hda)){median(x,na.rm=T)} else{0})
sum(hda1>thr,na.rm=T)#remove "24"
hda2<-apply(hda[,4:6],1,function(x) if(sum(is.na(x))<ncol(hda)){median(x,na.rm=T)} else{0})
sum(hda2>thr,na.rm=T)
hda3<-apply(hda[,7:9],1,function(x) if(sum(is.na(x))<ncol(hda)){median(x,na.rm=T)} else{0})
sum(hda3>thr,na.rm=T)
hdaM<-cbind(hda1,hda2,hda3)
...
```{r vennMedians}
Ctr<-apply(Ctrmedian,1,function(x) if(sum(is.na(x))<ncol(Ctrmedian)){median(x,na.rm=T)} else{0})
sum(Ctr>thr,na.rm=T)
Phy2p<-apply(Phy2pmedian,1,function(x) if(sum(is.na(x))<ncol(Phy2pmedian)){median(x,na.rm=T)} else{0})
sum(Phy2p>thr,na.rm=T)
Chol2p<-apply(Chol2pall,1,function(x) if(sum(is.na(x))<ncol(Chol2pall)){median(x,na.rm=T)} else{0})
sum(Chol2p>thr,na.rm=T)
dblDHA<-apply(dblDHAmedian,1,function(x) if(sum(is.na(x))<ncol(dblDHAmedian)){median(x,na.rm=T)} else{0})
sum(Phy2p>thr,na.rm=T)
hda<-cbind(Ctr,Chol2p,Phy2p,dblDHA)
hist(hda)
limma::vennDiagram(hda>thr)
writexl::write_xlsx(as.data.frame(cbind(Fasta=rownames(data),hda,Ctrmedian,Chol2pmedian,Phy2pmedian,dblDHAmedian)),paste0(inpF,hdr,selection,"median.xlsx"))
write.csv(cbind(Fasta=rownames(data),hda,Ctrmedian,Chol2pmedian,Phy2pmedian,dblDHAmedian),paste0(inpF,hdr,selection,"median.csv"),row.names = F)
```

```{r vennAll}
#warnings()
#Ctr<-apply(Ctrall[,-3],1,function(x) if(sum(is.na(x))<ncol(Ctrall)){median(x,na.rm=T)} else{0})
Ctr<-apply(Ctrall[,-3],1,function(x) if(sum(is.na(x))<ncol(Ctrall)){median(x,na.rm=T)} else{0})#remove "24"
sum(Ctr>thr,na.rm=T)
Phy2p<-apply(Phy2pall,1,function(x) if(sum(is.na(x))<ncol(Phy2pall)){median(x,na.rm=T)} else{0})
sum(Phy2p>thr,na.rm=T)
Chol2p<-apply(Chol2pall,1,function(x) if(sum(is.na(x))<ncol(Chol2pall)){median(x,na.rm=T)} else{0})
sum(Chol2p>thr,na.rm=T)
dblDHA<-apply(dblDHAall,1,function(x) if(sum(is.na(x))<ncol(dblDHAall)){median(x,na.rm=T)} else{0})
sum(Phy2p>thr,na.rm=T)
hda<-cbind(Ctr,Chol2p,Phy2p,dblDHA)
hist(hda)
limma::vennDiagram(hda>thr)
writexl::write_xlsx(as.data.frame(cbind(Fasta=rownames(data),hda,Ctrall,Chol2pall,Phy2pall,dblDHAall)),paste0(inpF,hdr,selection,"all.xlsx"))
write.csv(cbind(Fasta=rownames(data),hda,Ctrall,Chol2pall,Phy2pall,dblDHAall),paste0(inpF,hdr,selection,"all.csv"),row.names = F)
#warnings()
```

```{r tChol2p, echo = FALSE}
dataSellog2grpTtest<-cbind(Chol2pmedian,Ctrmedian)
comp<-"Ctr_Chol2p"
...
```{r tPhy2p, echo = FALSE}
dataSellog2grpTtest<-cbind(Phy2pmedian,Ctrmedian)
comp<-"Ctr_Phy2p"
dataSellog2grpTtest[dataSellog2grpTtest==0]=NA
summary(dataSellog2grpTtest)
row.names(dataSellog2grpTtest)<-row.names(data)
...
```{r tdblDHA, echo = FALSE}
dataSellog2grpTtest<-cbind(dblDHAmedian,Ctrmedian)
comp<-"Ctr_dblDHA"
dataSellog2grpTtest[dataSellog2grpTtest==0]=NA
summary(dataSellog2grpTtest)
row.names(dataSellog2grpTtest)<-row.names(data)
...
par(mfrow=c(1,2))
#library/packages
inpD <-"L:/promec/Qexactive/LARS/2021/march/Rolf/Salmon/"
fName<-"210223_ROLF_1-(4).xlsx"
