...
write_xlsx(as.data.frame(cbind(log2LFQ,rownames(data))),inpW)
```
...
```{r AQ3 AQ3 NC154 NC163 NC164 NC165 NC166 TT07 TT08 TT09, echo = FALSE}
hdr<-"AQ.3|plasma"
chkr<-56#nrow(dataSellog2grpTtest)
#dataSellog2grpTtest<-as.data.frame(log2LFQ[chkr,grep(hdr,colnames(log2LFQ))])
...
#as.numeric(quantile(c(22.39458934,20.48315841,21.87154733),na.rm=T)[5])
#as.numeric(quantile(c(22.39458934,20.48315841,21.87154733),na.rm=T)[1])-1.5*IQR(c(22.39458934,20.48315841,21.87154733))>max(as.numeric(c(NA,20.34494601)),na.rm=T)
dataSellog2grpTtest[chkr,c(1:mCol)]
dataSellog2grpTtest[chkr,c((mCol+1):eCol)]
sum(!is.na(dataSellog2grpTtest[chkr,c(1:eCol)]))
t.test(as.numeric(dataSellog2grpTtest[chkr,c(1:mCol)]),as.numeric(dataSellog2grpTtest[chkr,c((mCol+1):eCol)]),na.rm=T)$p.value
dim(dataSellog2grpTtest)
dim(dataSellog2grpTtest)[1]
pValNA = apply(
  dataSellog2grpTtest, 1, function(x)
    if(sum(!is.na(x[c(1:mCol)]))<2&sum(!is.na(x[c((mCol+1):eCol)]))<2){NA}
    #else if(sum(!is.na(x[c(1:mCol)]))==1&sum(!is.na(x[c((mCol+1):eCol)]))>=2){1}
    else if(sum(!is.na(x[c(1:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))==1&(as.numeric(quantile(as.numeric(x[c(1:mCol)]),na.rm=T)[1])-(1.5*IQR(as.numeric(x[c(1:mCol)]),na.rm=T)))>max(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)){0}
    else if(sum(!is.na(x[c(1:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))==1&(as.numeric(quantile(as.numeric(x[c(1:mCol)]),na.rm=T)[5])+(1.5*IQR(as.numeric(x[c(1:mCol)]),na.rm=T)))<min(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)){0}
    else if(sum(!is.na(x[c(1:mCol)]))==1&sum(!is.na(x[c((mCol+1):eCol)]))>=2&(as.numeric(quantile(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)[1])-(1.5*IQR(as.numeric(x[c((mCol+1):eCol)]),na.rm=T))>max(as.numeric(x[c(1:mCol)]),na.rm=T))){0}
    else if(sum(!is.na(x[c(1:mCol)]))==1&sum(!is.na(x[c((mCol+1):eCol)]))>=2&(as.numeric(quantile(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)[5])+(1.5*IQR(as.numeric(x[c((mCol+1):eCol)]),na.rm=T))<min(as.numeric(x[c(1:mCol)]),na.rm=T))){0}
    else if(sum(!is.na(x[c(1:mCol)]))>1&sum(!is.na(x[c((mCol+1):eCol)]))<1){0}
    else if(sum(!is.na(x[c(1:mCol)]))<1&sum(!is.na(x[c((mCol+1):eCol)]))>1){0}
    else if((sum(!is.na(x[c((mCol+1):eCol)]))==1)|(sum(!is.na(x[c(1:mCol)]))==1)){NA}
    else{
      #print(x)
      t.test(as.numeric(x[c(1:mCol)]),as.numeric(x[c((mCol+1):eCol)]),na.rm=T,var.equal=T)$p.value})
warnings()
hist(pValNA)
...
write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,hdr,"tTestBHgrpOut25pQ.xlsx"))
#write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,"wTestBHgrp.xlsx"))
```
