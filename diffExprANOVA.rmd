...
```{r impLFQHeatMap}
#select c(2,3,5,6,8,10:12) of KO/WT, impute 13 and pheatmap with scale row
log2LFQ13<-log2LFQ
log2LFQ13<-log2LFQ13[,c(2,3,5,6,8,10:12)]
summary(log2LFQ13)
naCnt <- apply(log2LFQ13, 1, function(x){all(is.na(x))})
summary(naCnt)
log2LFQ13 <- log2LFQ13[!naCnt,]
#log2LFQ13<-log2LFQ13[complete.cases(log2LFQ13), ]
summary(log2LFQ13)
log2LFQ13[is.na(log2LFQ13)]<-13
summary(log2LFQ13)
#dsub[dsub==13]<-NA
#dsub[is.na(dsub)]<-13
#install.packages("svglite")
p<-pheatmap::pheatmap(log2LFQ13,scale="row",clustering_distance_rows="correlation",clustering_distance_cols = "correlation",cluster_cols=T,cluster_rows=T,fontsize_col=6,fontsize_row=1)
ggplot2::ggsave(paste0(inpF,hdr,selection,"log2HeatMap.svg"), p)
```
