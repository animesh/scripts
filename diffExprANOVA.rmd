...
label<-read.table(paste0(getwd(),"/Heidi/annodata.txt"),sep="\t",header=T)
...
#colnames(label)
summary(label)
rownames(label)<-label[['axisLabels']]
#rownames(label)<-label[['Names']]
rownames(label)
#table(label)
```

```{r beads}
hda<-as.matrix(log2LFQ[,grep("hrk",colnames(log2LFQ))])
hist(as.matrix(hda))
#AQ3 NC154 NC163 NC164 NC165 NC166 TT07 TT08 TT09
AQ3<-apply(hda,1,function(x) if(sum(is.na(x[c(1:3)]))==0){median(x[c(1:3)],na.rm=T)} else{0})
sum(AQ3>0)
NC154<-apply(hda,1,function(x) if(sum(is.na(x[c(4:6)]))<5){median(x[c(4:6)],na.rm=T)} else{0})
sum(NC154>0)
NC164<-apply(hda,1,function(x) if(sum(is.na(x[6:8]))==0){median(x[6:8])} else{0})
sum(NC164>0)
NC165<-apply(hda,1,function(x) if(sum(is.na(x[6:8]))==0){median(x[6:8])} else{0})
sum(NC165>0)
NC166<-apply(hda,1,function(x) if(sum(is.na(x[6:8]))<3){median(x[6:8],na.rm=T)} else{0})
sum(NC166>0)
TT07<-apply(hda,1,function(x) if(sum(is.na(x[9:11]))==0){median(x[9:11])} else{0})
sum(TT07>0)
TT08<-apply(hda,1,function(x) if(sum(is.na(x[9:11]))<3){median(x[9:11],na.rm=T)} else{0})
sum(TT08>0)
TT09<-apply(hda,1,function(x) if(sum(is.na(x[12:14]))==0){median(x[12:14])} else{0})
sum(TT09>0)
NC163<-apply(hda,1,function(x) if(sum(is.na(x[12:14]))<3){median(x[12:14],na.rm=T)} else{0})
sum(NC163>0)
hdaM=cbind(AQ3,NC154,NC163,NC164,NC165,NC166,TT07,TT08,TT09)
...
```{r AQ3, echo = FALSE}
hdr<-"AQ.3|plasma"
dataSellog2grpTtest<-log2LFQ[,grep(hdr,colnames(log2LFQ))]
#dataSellog2grpTtest<-dataSellog2grpTtest[,-ncol(dataSellog2grpTtest)]
...
pValNA = apply(
  dataSellog2grpTtest, 1, function(x)
    if(sum(!is.na(x[c(sCol:mCol)]))<2&sum(!is.na(x[c((mCol+1):eCol)]))<2){NA}
    else if(sum(!is.na(x[c(sCol:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))==1&(as.numeric(quantile(as.numeric(x[c(sCol:mCol)]),na.rm=T)[1])-(iqThr*IQR(as.numeric(x[c(sCol:mCol)]),na.rm=T)))>max(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)){0}
    else if(sum(!is.na(x[c(sCol:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))==1&(as.numeric(quantile(as.numeric(x[c(sCol:mCol)]),na.rm=T)[5])+(iqThr*IQR(as.numeric(x[c(sCol:mCol)]),na.rm=T)))<min(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)){0}
    else if(sum(!is.na(x[c(sCol:mCol)]))==1&sum(!is.na(x[c((mCol+1):eCol)]))>=2&(as.numeric(quantile(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)[1])-(iqThr*IQR(as.numeric(x[c((mCol+1):eCol)]),na.rm=T))>max(as.numeric(x[c(sCol:mCol)]),na.rm=T))){0}
    else if(sum(!is.na(x[c(sCol:mCol)]))==1&sum(!is.na(x[c((mCol+1):eCol)]))>=2&(as.numeric(quantile(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)[5])+(iqThr*IQR(as.numeric(x[c((mCol+1):eCol)]),na.rm=T))<min(as.numeric(x[c(sCol:mCol)]),na.rm=T))){0}
    else if(sum(!is.na(x[c(sCol:mCol)]))>1&sum(!is.na(x[c((mCol+1):eCol)]))<1){0}
    else if(sum(!is.na(x[c(sCol:mCol)]))<1&sum(!is.na(x[c((mCol+1):eCol)]))>1){0}
    else if(sum(!is.na(x[c(sCol:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))==1&(as.numeric(quantile(as.numeric(x[c(sCol:mCol)]),na.rm=T)[1])-(iqThr*IQR(as.numeric(x[c(sCol:mCol)]),na.rm=T)))<=max(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)){1}
    else if(sum(!is.na(x[c(sCol:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))==1&(as.numeric(quantile(as.numeric(x[c(sCol:mCol)]),na.rm=T)[5])+(iqThr*IQR(as.numeric(x[c(sCol:mCol)]),na.rm=T)))>=min(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)){1}
    else if(sum(!is.na(x[c(sCol:mCol)]))==1&sum(!is.na(x[c((mCol+1):eCol)]))>=2&(as.numeric(quantile(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)[1])-(iqThr*IQR(as.numeric(x[c((mCol+1):eCol)]),na.rm=T))<=max(as.numeric(x[c(sCol:mCol)]),na.rm=T))){1}
    else if(sum(!is.na(x[c(sCol:mCol)]))==1&sum(!is.na(x[c((mCol+1):eCol)]))>=2&(as.numeric(quantile(as.numeric(x[c((mCol+1):eCol)]),na.rm=T)[5])+(iqThr*IQR(as.numeric(x[c((mCol+1):eCol)]),na.rm=T))>=min(as.numeric(x[c(sCol:mCol)]),na.rm=T))){1}
    else if(sum(!is.na(x[c(sCol:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))>=2){
      t.test(as.numeric(x[c(sCol:mCol)]),as.numeric(x[c((mCol+1):eCol)]),na.rm=T,var.equal=F)$p.value}
  else{NA}
  )
...
hdr<-gsub("[^[:alnum:] ]", "",hdr)
hist(log2FCmedianFC,main = hdr)
Gene<-paste(sapply(strsplit(paste(sapply(strsplit(rownames(dataSellog2grpTtest), ";;"), "[", 2)), ";"), "[", 1))
Uniprot<-paste(sapply(strsplit(paste(sapply(strsplit(rownames(dataSellog2grpTtest), ";;"), "[", 3)), ";"), "[", 1))
Uni<-paste(sapply(strsplit(Uniprot, "-"), "[", 1))
ttest.results = data.frame(RowGeneUniProtScorePeps=rownames(data),Gene=Gene, Uniprot=Uniprot,PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,welchTtestPval=pValNA,dataSellog2grpTtest,Log2MedianChange=logFCmedian,UnIso=Uni)
writexl::write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,hdr,"wTestBHgrpOut25pQ.xlsx"))
#write.csv(ttest.results,paste0(inpF,selection,sCol,eCol,hdr,"wTestBHgrpOut25pQ.csv"))
<<<<<<< HEAD
write.table(ttest.results,sep="\t",row.names=F,paste0(inpF,selection,sCol,eCol,hdr,"wTestBHgrpOut25pQ.txt"))
=======
write.table(ttest.results,sep="\t",paste0(inpF,selection,sCol,eCol,hdr,"wTestBHgrpOut25pQ.txt"))
>>>>>>> 2bb3d60028c708a3e19dbe8e520e659284d12bf9
limma::vennDiagram(cbind(logFCmedianGrp1,logFCmedianGrp2))
assign(paste0('data', hdr,"wTest"),ttest.results)
```
...
hdr<-"TT.07|plasma"
dataSellog2grpTtest<-log2LFQ[,grep(hdr,colnames(log2LFQ))]
dataSellog2grpTtest<-dataSellog2grpTtest[,c(4:6,1:3)]
#dataSellog2grpTtest<-dataSellog2grpTtest[,-ncol(dataSellog2grpTtest)]
summary(dataSellog2grpTtest)
row.names(dataSellog2grpTtest)<-row.names(data)
sCol<-1
