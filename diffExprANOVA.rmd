...
inpF<-"L:/promec/Qexactive/Mirta/EYE_PRPF31/20240127_PRPF31_DevelopmentalStudy/combined/txt/proteinGroups.txt"
...
```{r label}
label<-data.frame(Bio=sapply(strsplit(colnames(log2LFQ),"_",fixed=T), "[", 1))
rownames(label)<-colnames(log2LFQ)
annoFactor<-label["Bio"]
names(annoFactor)<-"Bio"
anno<-data.frame(factor(label[,"Bio"]))
row.names(anno)<-rownames(label)
names(anno)<-"Bio"
table(anno)
label["rGroup"]<-rep(NA,nrow(label))
...
```{r pgClust}
bk1 <- c(seq(range(log2LFQ,na.rm=T)[1],range(log2LFQ,na.rm=T)[1]+(range(log2LFQ,na.rm=T)[2]-range(log2LFQ,na.rm=T)[1])/2,by=0.01))
bk2 <- c(seq(range(log2LFQ,na.rm=T)[1]+(range(log2LFQ,na.rm=T)[2]-range(log2LFQ,na.rm=T)[1])/2,range(log2LFQ,na.rm=T)[2],by=0.01))
bk <- c(bk1,bk2)  #combine the break limits for purpose of graphing
palette <- c(colorRampPalette(colors = c("green", "black"))(n = length(bk1)-1),"black", "black",c(colorRampPalette(colors = c("black","red"))(n = length(bk2)-1)))
log2LFQO<-log2LFQ[order(rowSums(log2LFQ,na.rm=T),decreasing = T),]
svgPHC<-pheatmap::pheatmap(log2LFQO,color = palette,clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean",fontsize_row=1,cluster_cols=T,cluster_rows=F,fontsize_col  = 4,na_col = "grey",annotation_col = annoR)
ggplot2::ggsave(file=paste0(inpF,"log2Clust.svg"),plot=svgPHC,width=6, height=8,dpi=320)#,  
svgPHC<-pheatmap::pheatmap(log2LFQO,clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean",fontsize_row=1,cluster_cols=T,cluster_rows=F,fontsize_col  = 4,na_col = "white",annotation_col = annoR)
ggplot2::ggsave(file=paste0(inpF,"log2ClustRYBW.svg"),plot=svgPHC,width=6, height=8,dpi = 320)#,  
svgPHC<-pheatmap::pheatmap(log2LFQ,color = palette,clustering_distance_rows = "canberra",clustering_distance_cols = "canberra",fontsize_row=4,cluster_cols=T,cluster_rows=F,fontsize_col  = 4,na_col = "grey",annotation_col = annoR)
svgPHC<-pheatmap::pheatmap(log2LFQ,color = palette,clustering_distance_rows = "binary",clustering_distance_cols = "binary",fontsize_row=4,cluster_cols=T,cluster_rows=F,fontsize_col  = 4,na_col = "grey",annotation_col = annoR)
ggplot2::ggsave(file=paste0(inpF,"log2.svg"),plot=svgPHC,width=6, height=10,dpi = 320)#,  
...
svgPHC<-pheatmap::pheatmap(countTableDAuniGORNAdds,clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean",fontsize_row=1,cluster_cols=T,cluster_rows=F,fontsize_col  = 8,na_col = "grey")
countTableDAuniGORNAdds<-(log2LFQ-countTableDAuniGORNAddsMed)#/countTableDAuniGORNAddsSD
hist(countTableDAuniGORNAdds)
par(mar=c(12,3,1,1))
boxplot(countTableDAuniGORNAdds,las=2)
...
vsn::meanSdPlot(log2LFQselMMdata,rank=F)
...
write.csv(log2LFQselMMdata,paste0(inpF,"log2LFQselMMdata.csv"),row.names = T,quote = F)
svgPHC<-pheatmap::pheatmap(log2LFQselMMdata,clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean",fontsize_row=8,cluster_cols=T,cluster_rows=F,fontsize_col  = 8,na_col = "grey")
ggplot2::ggsave(file=paste0(inpF,"log2LFQselMMdata.svg"),plot=svgPHC,width=6, height=10,dpi = 320)#,  
corlog2LFQselMMdata<-cor(log2LFQselMMdata,use="complete.obs")
svgPHC<-pheatmap::pheatmap(corlog2LFQselMMdata,clustering_distance_rows = "correlation",clustering_distance_cols = "correlation",fontsize_row=6,cluster_cols=T,cluster_rows=T,fontsize_col  = 6,na_col = "grey")
ggplot2::ggsave(file=paste0(inpF,"corlog2LFQselMMdata.svg"),plot=svgPHC,width=6, height=10,dpi = 320)#,  
```
