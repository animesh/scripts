...
```{r merge}
#git checkout 8720abd7bcaef40f536191b512cf3ff1b2c80ce1 diffExprANOVA.rmd
inpD <-"F:/promec/Animesh/Andi/"
fName<-"MS-Combined.xlsx"
inpF<-paste0(inpD,fName)
data <- readxl::read_xlsx(inpF)
data[1,]
Ogg1NT<-as.data.frame(data[,c(1,2)])
colnames(Ogg1NT)<-Ogg1NT[1,]
Ogg1H2O2<-as.data.frame(data[,c(4,5)])
colnames(Ogg1H2O2)<-Ogg1H2O2[1,]
MutyhNT<-as.data.frame(data[,c(7,8)])
colnames(MutyhNT)<-MutyhNT[1,]
MutyhH2O<-as.data.frame(data[,c(10,11)])
colnames(MutyhH2O)<-MutyhH2O[1,]
ID=cbind(t(Ogg1NT$ID),t(Ogg1H2O2$ID),t(MutyhNT$ID),t(MutyhH2O$ID))
ID=unique(ID[1,])
ID=na.omit(ID)
dataMerged<-data.frame(ID=ID)
rownames(dataMerged)<-dataMerged$ID
dataMerged<-merge(Ogg1NT,Ogg1H2O2,by="ID",all=T)
names(dataMerged)[2]<-"Ogg1NT"
names(dataMerged)[3]<-"Ogg1H2O2"
dataMerged<-merge(dataMerged,MutyhNT,by="ID",all=T)
dataMerged<-merge(dataMerged,MutyhH2O,by="ID",all=T)
names(dataMerged)[4]<-"MutyhNT"
names(dataMerged)[5]<-"MutyhH2O"
dataVenn<-dataMerged[,c(2:5)]
dataVenn[is.na(dataVenn)]<-0
limma::vennDiagram(dataVenn>0)
#writexl::write_xlsx(as.data.frame(dataMerged),"dataMerged.xlsx")
write.csv(as.data.frame(dataMerged),paste0(inpF,"dataMerged.csv"))
#ID=union(unlist(data[,c(1,4,7,10)]))
#for (obj in c(Ogg1NT,Ogg1H2O2,MutyhNT,MutyhH2O)){ print(obj) dataMerged<-merge(dataMerged,get(obj),by="ID",all=T)}
```
