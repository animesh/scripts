...
#!/usr/bin/perl
#final NCDSE extractor based on the chi stat
...
$t11=@t1[0];$t11=~s/\>| //g;chomp $t11;
if($t11 eq "cIntergenic"){
#print "cINTERGENIC\n";
#$st=@t1[2]-@t1[9]+1-3;$sp=@t1[2]-@t1[6]+1;$length=$sp-$st+1;$str = uc(substr($line,($st-1),($length)));
$str = reverse ($str);
$str =~ tr/ATCG/TAGC/d;
if($str=~/^(TTG|GTG|ATG)/ and $str=~/(TAA|TAG|TGA)$/ and $length >= 120){
print "@t1[0]\[$st-$sp]\t$t1[4]\t$length\t$lto{$w}\t$file\n$str\n";
}
}
else{
if($str=~/^(TTG|GTG|ATG)/ and $str=~/(TAA|TAG|TGA)$/and $length >= 120){
