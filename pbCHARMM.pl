...
if (defined $customfile && &GenUtil::checkFile($customfile)) {
  ...
  if (defined $emap) {
    ...
    $charmm->{par}->{epsw}=1;
    my ($tmaxx,$tmaxy,$tmaxz,$nxcel,$nycel,$nzcel)=$charmm->pbgrid("$$.eps1","phi"); 
    &convert2DX($tmaxx,$tmaxy,$tmaxz,$nxcel,$nycel,$nzcel,$charmm->{par}->{dcel},"$$.eps1");
    $charmm->{par}->{epsw}=$saveeps;
    ($tmaxx,$tmaxy,$tmaxz,$nxcel,$nycel,$nzcel)=$charmm->pbgrid("$$.epsw","phi"); 
    &convert2DX($tmaxx,$tmaxy,$tmaxz,$nxcel,$nycel,$nzcel,$charmm->{par}->{dcel},"$$.epsw");
    ...
    if (defined $gridname) {
      my ($tmaxx,$tmaxy,$tmaxz,$nxcel,$nycel,$nzcel)=$charmm->pbgrid($epsgrid,$gridname); 
      &convert2DX($tmaxx,$tmaxy,$tmaxz,$nxcel,$nycel,$nzcel,$charmm->{par}->{dcel},$epsgrid) if ($dx);
    } else {
...
sub convert2DX {
  ...
  printf OUT "origin %e %e %e\n",-($nx/2.0-0.5)*$dcel,-($ny/2.0-0.5)*$dcel,-($nz/2.0-0.5)*$dcel;
