...
inpF = sys.argv[1]
columnID = sys.argv[2]
#inpF = "L:\\promec\\TIMSTOF\\LARS\\2022\\januar\\220119_Elise_rerun\\combined\\txt\\msms.txt"
#columnID = "Raw file"
#rowID = "Proteins"
#valID = 'Score'
import pandas as pd
...
print(df.head())
#print(df.info())
df['indexID']=df[rowID]+df[columnID]
#dfS=df.groupby('indexID').sum()
dfS=df.groupby([columnID,rowID]).size()
dfS=df.groupby([columnID,rowID]).count()
#dfS=df.pivot(index='indexID', columns=columnID, values=valID)
dfC=dfS.groupby(columnID).count()
...
dfC.plot(kind="barh").figure.savefig(outFc,dpi=100,bbox_inches = "tight")
#plt.close()
print(outFc)
