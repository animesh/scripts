...
#install.packages("BiocManager", repos="http://cran.r-project.org", lib="C:/Users/animeshs/GD/R_libs")
#library("BiocManager",lib="C:/Users/animeshs/GD/R_libs")
#BiocManager::install("limma",lib="C:/Users/animeshs/GD/R_libs")
#library("limma")#,lib="C:/Users/animeshs/GD/R_libs")
getwd()
#setwd("C:/Users/animeshs/GD/")
#install.packages("readxl", repos="http://cran.r-project.org", lib="C:/Users/animeshs/GD/R_libs")
#library("readxl",lib="C:/Users/animeshs/GD/R_libs")
#inpF<-paste0(inpD,"proteinGroupsLog2diffs.xlsx")
#data<-read_xlsx(inpF)
...
data$Uniprot<-sapply(strsplit(as.character(data$Protein.IDs),"[-;]"), "[", 1)
summary(data)
...
hist(as.matrix(log2(data[,grep("LFQ",colnames(data))])))
writexl::write_xlsx(as.data.frame(cbind(data$Uniprot,log2LFQ)),paste0(inpF,"uniprot.xlsx"))
```
...
dataSezWP <- read.csv(inpOm)
summary(dataSezWP)
#dataS<-as.matrix(data[,1:4],rownames=T)
dataSezWPs<-as.matrix(dataSezWP[,c(1:6)])
hist(dataSezWPs)
dataSezWPs[dataSezWPs==0]=NA
dataSezWPs[is.na(dataSezWPs)]=1
hist(dataSezWPs)
rn<-sub(" ",";",unlist(dataSezWP[32]))
rn<-strsplit(rn, ";")
ln<-1:nrow(dataSezWPs)
rn<-paste(sapply(rn, "[", 1),sapply(rn, "[", 2),ln,sep=";")#,sapply(rn, "[", 3),sapply(rn, "[", 6))
row.names(dataSezWPs)<-rn
cn<-strsplit(colnames(dataSezWPs), "_")
colnames(dataSezWPs)<-paste(sapply(cn, "[", 1))#,sapply(cn, "[", 4))
summary(dataSezWPs)
hist(dataSezWPs)
limma::vennDiagram(dataSezWPs[,c(1:3,6)]<selThr,main="wikiD",counts.col = "red")
```
...
```{r geneList, echo = FALSE}
#ORA: ORA’s propensity for type I error rate inflation when genes tend to be co-expressed within sets
list11<-as.data.frame(toupper(c("TPD52", "BCM", "LILRB4", "FADS2", "C11orf58", "GAA", "CGREF1", "TSG101", "ZNRF2", "COG4", "UPF3B")))
colnames(list11)<-"gene"
summary(list11)
```
...
wikiWP2369 <- matrix(c(3,13,3663,1293), nrow = 2,dimnames =list(wikiP=c("full", "selected"),pWiki=c("selected", "other")))
wikiWP185 <- matrix(c(13,16,3806,1137), nrow = 2,dimnames =list(wikiP=c("full", "selected"),pWiki=c("selected", "other")))
wikiWP2369<-wikiWP185
sum(wikiWP2369)
...
wikiP<-rbind(c(6,622),c(11,4447))
sum(wikiP)
fisher.test(wikiP, alternative = "less")
fisher.test(wikiP)
fisher.test(wikiP, conf.level = 0.95)$conf.int
...
dataSez<-merge(sort=F,dataS, uniprots2entrez, by.x="T..Uniprot", by.y="UNIPROT")
dataSez<-merge(sort=F,dataS, uniprots2entrez, by.x="T..Entry", by.y="UNIPROT")
