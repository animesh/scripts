...
fisher.test(rbind(c(1,9),c(11,3)))$p.value/2
...
(choose(628,6)*choose(4458,11))/(choose(5086,17))
...
#https://youtu.be/O182Gr97-DI?t=3922
wikiP <- matrix(c(2,1,1000,1000), nrow = 2,dimnames =list(wikiP=c("full", "selected"),pWiki=c("selected", "other")))
fisher.test(wikiP)
wikiP <- matrix(c(20,10,1000,1000), nrow = 2,dimnames =list(wikiP=c("full", "selected"),pWiki=c("selected", "other")))
fisher.test(wikiP)
wikiP <- matrix(c(200,100,1000,1000), nrow = 2,dimnames =list(wikiP=c("full", "selected"),pWiki=c("selected", "other")))
fisher.test(wikiP)
wikiP <- matrix(c(2,10,5000,5000), nrow = 2,dimnames =list(wikiP=c("full", "selected"),pWiki=c("selected", "other")))
fisher.test(wikiP)
```
```{r rapaB}
#https://www.uniprot.org/uniprot/?query=taxonomy%3A%22Brassica+rapa+%28field+mustard%29+%5B3711%5D%22&sort=score
```
```{r ratData}
fileI<-"L:/promec/Animesh/Atle/Heidelberg/Q25D165-17-27-01-(1)_Proteins.txt"
data<-read.table(fileI,header = T)
table(unlist(strsplit(data$Cellular.Component, split = ";")))
dataM<-data.frame(data[data$Master=="IsMasterProtein",])
hist(dataM$Abundance.Ratio.log2.Serotonin..Control)
hist(dataM$Abundance.Ratio.P.Value.Serotonin..Control)
hist(dataM$Abundance.Ratio.Adj.P.Value.Serotonin..Control)
table(unlist(strsplit(dataM$Cellular.Component, split = ";")))
dataS<-dataM[dataM$Abundance.Ratio.log2.Serotonin..Control>9&!is.na(dataM$Abundance.Ratio.log2.Serotonin..Control),]#&dataM$Abundance.Ratio.Adj.P.Value.Serotonin..Control<0.01,]
table(unlist(strsplit(dataS$Cellular.Component, split = ";")))
dataC<-dataM[dataM$Abundance.Ratio.log2.Serotonin..Control<(-9)&!is.na(dataM$Abundance.Ratio.log2.Serotonin..Control),]#&dataM$Abundance.Ratio.Adj.P.Value.Serotonin..Control<0.01,]
table(unlist(strsplit(dataC$Cellular.Component, split = ";")))
(choose(105,5)*choose(7471,899))/(choose(7576,904))#https://www.wolframalpha.com/input/?i=%28choose%28105%2C5%29*choose%287471%2C899%29%29%2F%28choose%287576%2C904%29%29 for cytoskeleton
fisher.test(rbind(c(5,100),c(899,6572)), alternative="less")$p.value
```
...
data( proteome)
data( metabolome)
# Initialize the omics data structure
initOmicsDataStructure <- function(layer) {
  omics_data <- list()
  for (l in layer) {
    omics_data[[l]] <- list()
  }
...
rankFeatures <- function(logFC, pValue) {
  ranked_features <- -log10(pValue) * sign(logFC)
