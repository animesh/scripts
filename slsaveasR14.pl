...
sub pr
{
    ...
    print $indent;
    print $rule->{"name"}, " | ", $rule->{"val"}, " : ", $rule->{"action"}, "\n";
    ...
    ORSET: foreach $orset (@{$rules_ref->{"subs"}}){
        ...
        RULE: foreach $subrule (@{$orset}){
            ...
            OBJ: foreach $subobj (@{$objref->{"subs"}}){
                ...
                if($subobj->{"name"} eq $subrule->{"name"}
                    || $subrule->{"name"} eq "WILDCARD"){
...
#     insertsib
#          places the last removed object as a younger
#          sibling of the current object.
#     insert
#          insert the last removed object as a 
...
#    
# Unimplemented:
#   There is a case for addnewpair and addnewcontainer
#   actions but holding off on creating them until they 
...
sub  read_rules()
{
    ...
    foreach $line ( @{ $lines_ref }){
        ...
        if(@toks){  #skip proccessing for empty lines
            ...
            shift(@toks); # shift off the first '<'
