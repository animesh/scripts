...
if (!defined $readlog) {  
  ...
  my $cmd;
  if ($db eq "pdb") {
    $cmd=sprintf("%s -M %s %s -i %s -d nr -j %d -b %d -e %f -h %f -C $$.pssm >& $$.blastoutnr\n",$blastpgp,$matrix,$gapstr,"$$.seq",$niter,$nalign,$eshow,$ekeep);
    system($cmd);
    $cmd=sprintf("%s -M %s %s -i %s -d pdbaa -j 1 -b %d -e %f -R $$.pssm >& $$.blastout",$blastpgp,$matrix,$gapstr,"$$.seq",$nalign,$eshow,(defined $logfile)?$logfile:"$$.blastout");
  } else {
    $cmd=sprintf("%s -M %s %s -i %s -d %s -j %d -b %d -e %f >& $$.blastout",$blastpgp,$matrix,$gapstr,"$$.seq",$db,$niter,$nalign,$eshow);
  }
