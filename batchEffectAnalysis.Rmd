```{r data}
inpF <-"F:/promec/Animesh/Caroline/proteinGroupsLog2LFQimp1p8dfM3pSD.txt"
data <- read.delim(inpF,row.names=27,sep="\t",header = T)
summary(data)
dataNorm=log2(data[,grep("X", names(data))])
...
```
```{r comp}
plot(prcomp(dataNorm))
biplot(prcomp(dataNorm,scale = F))
biplot(princomp(dataNorm))
```
