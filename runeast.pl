for($c=0;$c<7;$c++){
	...
	system("time ./flowsim-0.2.5  -G Titanium --model=empdist.ani.smooth.txt     --degrad=\"Normal 0 0\"  -n $nt $seabass -o $seabass.$c.$nt.$titanium.east.800.sff");
	print "Assembly\n";
	system("time /space/animesh/site/454apps/bin/runAssembly -o $seabass.$c.$nt.$titanium.800.east.RA -g $seabass.$c.$nt.$titanium.east.800.sff");
	print "Mapping\n";
	system("time /space/animesh/site/454apps/bin/runMapping  -o $seabass.$c.$nt.$titanium.800.east.RM $seabass $seabass.$c.$nt.$titanium.east.800.sff");
	print "\n";
