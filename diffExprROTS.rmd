```{r proteinGroups}
inpD <-"Z:/PA/_Pseudomonas_ 1/"
inpF<-paste0(inpD,"140605_Pseudomonas_O1_K0K6_Proteins.txt")
data <- read.delim(inpF,sep="\t",header = T,row.names = 5)
data = data[!data$Contaminant=="True",]
summary(data)
dim(data)
colnames(data)
selection<-"Abundance.Ratio.log2."
SILAC<-as.matrix(data[,grep(selection,colnames(data))])
colnames(SILAC)<-sub(selection,"",colnames(SILAC))
colnames(SILAC)
dim(SILAC)
log2SILAC<-SILAC#log2(SILAC)
log2SILAC[log2SILAC==-Inf]=NA
dHA<-as.matrix(log2SILAC[,grep("F",colnames(log2SILAC))])
hist(as.matrix(dHA))
SILACall<-apply(dHA,1,function(x) if(sum(is.na(x))==0){median(x,na.rm=T)} else{0})
sum(SILACall>0)
SILACmid<-apply(dHA,1,function(x) if(sum(is.na(x))<length(x)/2){median(x,na.rm=T)} else{0})
sum(SILACmid>0)
SILACany<-apply(dHA,1,function(x) if(sum(is.na(x))<length(x)){median(x,na.rm=T)} else{0})
sum(SILACany>0)
hist(as.matrix(SILACany))
plot(SILACany,SILACmid)
hdaMall<-cbind(SILACany,SILACall,SILACmid)
...
gene<-toupper(as.character(data$Gene.Symbol))
gen<-as.character(sapply(strsplit(gene,";"), `[`, 1))
protein<-as.character(data$Description)
summary(hdaMall)
library("limma")
vennDiagram(hdaMall>0)
inpW<-paste0(inpF,"sel-SILAC.xlsx")
library(writexl)
write_xlsx(as.data.frame(cbind(uniprot,hdaMall,Gene=gen,Fasta=data[,6])),inpW)
```
...
```{r data}
inpD<-"F:/GD/"
inpF<-paste0(inpD,"Log2SILACwithBasicStats - 05.04.20 (676 protein groups) - to Animesh.xlsx")
...
library("writexl")
inpW<-paste0(inpD,"Copy of Log2SILACwithBasicStats - 15.10.18 (cut off 18) Label.write.xlsx")
write_xlsx(data,inpW)
```

```{r label}
inpL<-paste0(inpD,"Copy of Log2SILACwithBasicStats - 15.10.18 (cut off 18) Label.xlsx")
...
VarianceScatterplot(fit4, xlab="log2(SILAC)")
