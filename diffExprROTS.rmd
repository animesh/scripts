```{r data, echo = FALSE}
inpD <-"L:/promec/Qexactive/Mirta/QExactive/CLN3/RawData/DDA/combined/txtLFQ/"
...
protNum<-1:ncol(LFQ)
#colnames(LFQ)=paste(protNum,sub(selection,"",colnames(LFQ)),protNum,sep=".")
dim(LFQ)
log2LFQ<-log2(LFQ)
...
write_xlsx(as.data.frame(cbind(unipro,log2LFQ)),inpW)
```

```{r label}
inpL<-paste0(inpD,"Class.xlsx")
label<-read_excel(inpL)#,header=T,row.names=1,sep="\t")#, colClasses=c(rep("factor",3)))
...
dataNormImp=mice(as.matrix(log2LFQ), method="rf")
dataNormImpCom <- complete(dataNormImp,1)
row.names(dataNormImpCom)<-row.names(log2LFQ)
colnames(dataNormImpCom)<-colnames(log2LFQ)
summary(dataNormImpCom)
hist(as.matrix(dataNormImpCom))
```

```{r impute-shift, echo = FALSE}
scale=4
set.seed(scale)
...
hist(log2LFQimp,main=paste("Mean:",mean(log2LFQimp),"SD:",sd(log2LFQimp)),breaks=round(max(log2LFQ,na.rm=T)))
row.names(log2LFQimp)<-row.names(log2LFQ)
...
library(ROTS)
dataNormImpCom<-log2LFQimp
colnames(dataNormImpCom)
summary(dataNormImpCom)
hist(dataNormImpCom)
groups = label#$Code
groups
results = ROTS(data = dataNormImpCom[,c(1,2,3,5)], groups = as.numeric(as.factor(groups$Sub[c(1,2,3,5)])) , B = 250 , K = 1000 , seed = 42)
names(results) 
fdrThr<-0.01
...
```{r t-test-mut/ctr, echo = FALSE}
#lc1<-data[,label[label[["Comparisons"]]=="1",1]]
#dplyr::select(data,unlist(label[(label$Comparisons=="1")&(!is.na(label$Comparisons)),1]))
colnames(log2LFQ)
dataSellog2grpTtest<-log2LFQ[,gsub("\\s+|\\-", ".", unlist(label[(label$Test=="\"4-3\""),1]))]
dataSellog2grpTtest<-dataSellog2grpTtest[,c(2,3,1,4)]
sCol<-1
eCol<-4
mCol<-2
...
chk<-c(24.61419805,	23.30256976,	23.31962031, NA)	
chk<-c(24.61419805,	23.30256976,	23.31962031, 23.3196203)	
chk[c(1:mCol)]
chk[c((mCol+1):eCol)]
t.test(c(24.96987305,	24.95046975),c(24.16507321,	24.17714029),var.equal = F)
quantile(as.numeric(chk[c(1:mCol)]),probs=c(.025,.975),na.rm=T)[1]
quantile(as.numeric(chk[c(1:mCol)]),probs=c(.025,.975),na.rm=T)[2]
quantile(as.numeric(chk[c((mCol+1):eCol)]),probs=c(.025,.975),na.rm=T)[1]
quantile(as.numeric(chk[c((mCol+1):eCol)]),probs=c(.025,.975),na.rm=T)[2]
pValNA = apply(
  dataSellog2grpTtest, 1, function(x)
    if(sum(!is.na(x[c(1:eCol)]))<2){NA}
    else if(sum(!is.na(x[c(1:mCol)]))<2&sum(!is.na(x[c((mCol+1):eCol)]))<2){NA}
...
#    else if(sum(!is.na(x[c(1:mCol)]))>=2&sum(!is.na(x[c((mCol+1):eCol)]))==1&((quantile(as.numeric(x[c((mCol+1):eCol)]),probs=c(.025,.975),na.rm=T)[2])<min(as.numeric(x[c(1:mCol)]),na.rm=T))){0}
    ...
    else if(sum(!is.na(x[c(1:mCol)]))<2){NA}
    else if(sum(!is.na(x[c((mCol+1):eCol)]))<2){NA}
...
ttest.results = data.frame(RowGeneUniProtScorePeps=rownames(dataSellog2grpTtest), PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,TtestPval=pValNA,dataSellog2grpTtest,Log2MedianChange=logFCmedian,Uniprot=unipro)
#write.csv(ttest.results,file=paste0(inpF,hdr,sCol,eCol,"wTestBHgrp.csv"))
write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,"tTestBHgrp.xlsx"))
```

```{r t-test-child1/ctr, echo = FALSE}
#lc1<-data[,label[label[["Comparisons"]]=="1",1]]
#dplyr::select(data,unlist(label[(label$Comparisons=="1")&(!is.na(label$Comparisons)),1]))
colnames(log2LFQ)
dataSellog2grpTtest<-log2LFQ[,gsub("\\s+|\\-", ".", unlist(label[(label$Test=="\"2-1\""),1]))]
colnames(dataSellog2grpTtest)
dataSellog2grpTtest<-dataSellog2grpTtest[,c(1,2,3,5)]
colnames(dataSellog2grpTtest)
sCol<-1
eCol<-4
mCol<-2
...
ttest.results = data.frame(RowGeneUniProtScorePeps=rownames(dataSellog2grpTtest), PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,TtestPval=pValNA,dataSellog2grpTtest,Log2MedianChange=logFCmedian,Uniprot=unipro)
#write.csv(ttest.results,file=paste0(inpF,hdr,sCol,eCol,"wTestBHgrp.csv"))
write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,"tTestBHgrpChild1.xlsx"))
```

```{r t-test-child2/ctr, echo = FALSE}
#lc1<-data[,label[label[["Comparisons"]]=="1",1]]
#dplyr::select(data,unlist(label[(label$Comparisons=="1")&(!is.na(label$Comparisons)),1]))
colnames(log2LFQ)
dataSellog2grpTtest<-log2LFQ[,gsub("\\s+|\\-", ".", unlist(label[(label$Test=="\"2-1\""),1]))]
colnames(dataSellog2grpTtest)
dataSellog2grpTtest<-dataSellog2grpTtest[,c(4,6,3,5)]
colnames(dataSellog2grpTtest)
sCol<-1
eCol<-4
mCol<-2
...
ttest.results = data.frame(RowGeneUniProtScorePeps=rownames(dataSellog2grpTtest), PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,TtestPval=pValNA,dataSellog2grpTtest,Log2MedianChange=logFCmedian,Uniprot=unipro)
#write.csv(ttest.results,file=paste0(inpF,hdr,sCol,eCol,"wTestBHgrp.csv"))
write_xlsx(ttest.results,paste0(inpF,selection,sCol,eCol,"tTestBHgrpChild2.xlsx"))
```
...
dim(dTTd)
dim(dTTd)[1]
xxx<-c(-12.8099,-12.8099,NA)
which(!is.na(xxx))
xxx<-rbind(xxx,xxx)
t.test(xxx)
quantile(xxx,na.rm=T)
max(as.numeric(xxx[1]),na.rm=T)
min(xxx)
sd(xxx,na.rm=T)
length(xxx)
sum(is.na(xxx))
...
pValNA = apply(dTTd,1,function(x) 
  if(sum(!is.na(x))<2){1}
  else if(sd(x,na.rm=T)<=.Machine$double.xmin){0}
  else{t.test(x,na.rm=T)$p.value}
  )
hist(pValNA)
pValNAminusLog10 = -log10(pValNA+.Machine$double.xmin)
hist(pValNAminusLog10)
...
pValBHnaMinusLog10 = -log10(pValBHna+.Machine$double.xmin)
hist(pValBHnaMinusLog10)
logFCmedian = matrixStats::rowMedians(dTTd,na.rm=T)
hist(logFCmedian)
logFCmedianFC = 2^(logFCmedian+.Machine$double.xmin)
hist(logFCmedianFC)
logFCmedianFC=squish(logFCmedianFC,c(0.01,100))
...
ttest.results = data.frame(Uniprot=rowName,PValueMinusLog10=pValNAminusLog10,FoldChanglog2median=logFCmedianFC,CorrectedPValueBH=pValBHna,TtestPval=pValNA,dTTd,Plasma=dataSellog2grpTtest[,1],Log2MedianChange=logFCmedian,RowGeneUniProtScorePeps=rownames(dTTd))
...
pValNA = apply(dTTd,1,function(x) 
  ...
  else if(sum(!is.na(x[-length(x)]))<1&x[length(x)]<=0){1}
  else if(sum(!is.na(x[-length(x)]))>1&x[length(x)]==0){0}
  else if(sum(!is.na(x[-length(x)]))==1){1}
  else if(sum(!is.na(x[-length(x)]))>1&sd(x[-length(x)],na.rm=T)<(.Machine$double.xmin)){1}
...
logFCmedian[is.na(logFCmedian)]<-dTTd[is.na(logFCmedian),ncol(dTTd)]
hist(logFCmedian)
logFCmedianFC = 2^(logFCmedian+.Machine$double.xmin)
hist(logFCmedianFC)
logFCmedianFC=squish(logFCmedianFC,c(0.01,100))
