...
```{r label}
inpL<-"L:/promec/Qexactive/LARS/2019/spetember/Camilla Wolo/combined/txt/Groups.txt"
...
hdr="Ratio."
dataSel=as.matrix(data[,grep("normalized",colnames(data))])
dataSel=as.matrix(dataSel[,grep("pass",colnames(dataSel))])
colnames(dataSel)<-sub(,hdr,colnames(dataSel))
colnames(dataSel)
summary(dataSel)
```
...
```{r groups}
dataSellog2grp<-dataSellog2[,-grep("H.M.",colnames(dataSellog2))]
dataSellog2grp7<-dataSellog2grp[,grep("_7_",colnames(dataSellog2grp))]
summary(dataSellog2grp7)
dataSellog2grp11<-dataSellog2grp[,grep("_11_",colnames(dataSellog2grp))]
summary(dataSellog2grp11)
```

```{r t-test, echo = FALSE}
dataSellog2grpTtest<-dataSellog2grp7
...
ttest.results7 = data.frame(logFC=logFC,logFCmedian=logFCmedian,P.Value = pValNA, adj.pval.BH = pValBHna) 
...
#head(ttest.results)
write.csv(ttest.results7,file=paste0(inpF,hdr,"tTestBHgrp7.csv"))
```
