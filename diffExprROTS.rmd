```{r data}
inpD<-"L:/promec/Qexactive/LARS/2020/February/Heidi nano/combined/txt/"
inpF<-paste0(inpD,"proteinGroups.txt")
data<-read.table(inpF,header=T,sep="\t",row.names = 1)
row.names(data)<-paste(row.names(data),data[["Fasta.headers"]],sep=";")
summary(data)
```
...
rownames(label)<-gsub(" ",".",rownames(label))
rownames(label)<-paste0("LFQ_",rownames(label))
rownames(label)
```
...
sCol<-7
eCol<-12
mCol<-round((eCol-sCol+1)/2)
dataSellog2grpTtest<-dataSellog2[,sCol:eCol]
