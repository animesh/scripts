```{r}
pv<-c(0.00071993,0.001390152,0.001620303,0.014643094,0.014791889,0.026367867,1,1,1,1,1)
p.adjust(pv,method="BH")
```
```{r}
vals<-c(0:1000)
quantiles<-quantile(vals,probs=seq(0,1,0.01))
plot(quantiles)
```
```{r perm-ANOVA}
library(vegan)
...
library(readxl)
set.seed(100)
data<-readexcel("test.xlsx")
pANOVAdata<-data[,2:4]
pANOVAdatasqrt<-sqrt(pANOVAdata)
brayDistresult<-metaMDS (pANOVAdatasqrt, distance = "bray")
brayDistscores <-as.data.frame(scores(brayDistresult)$sites)
groupcentroids <- data.frame(Location = c("Lower", "Upper"),CentroidX = c(mean(brayDistscores$brayDist1[data$Location == "Lower"]),mean(brayDistscores$brayDist1[data$Location == "Upper"])),CentroidY = c(mean(brayDistscores$brayDist2[data$Location == "Lower"]),mean(brayDistscores$brayDist2[data$Location == "Upper"])))
plotdata<-data.frame(Location = data$Location,brayDist1=brayDistscores$brayDist1,brayDist2=brayDistscores$brayDist2,xend=c(rep( groupcentroids[1,2],10),rep(groupcentroids[2,2],10)),yend=c(rep(groupcentroids[1,3],10),rep( groupcentroids[2,3],10)))
ggplot(plotdata, aes(brayDist1,brayDist2)) + geompoint(aes(color = Location),size=2)+ statellipse(geom = "polygon", alpha = 0.05, aes(group = Location), color = "black",fill="blue")+ geompoint(data = groupcentroids, aes(x = CentroidX, y = CentroidY), color = "black", size = 2, shape = 7)+ geomsegment(data = plotdata, aes(x =brayDist1, y = brayDist2, xend = xend, yend = yend, color = Location), alpha = 0.5)+ scalecolormanual(name=" Locations",labels= unique(plotdata$Location),values= c("darkgreen", "darkred"))+ themebw()
pANOVAdist<-vegdist(pANOVAdatasqrt, method='bray')
dispersion<-betadisper(pANOVAdist, group=data$Location,type = "centroid")
plot(dispersion)
anova(dispersion)
pANOVAaresult<-adonis2( pANOVAdist~as.factor(plotdata$Location), data=pANOVAdist,pANOVAutations=9999)
print(pANOVAaresult)
```

```{r AAA-FDRcheck}
#https://tools.carbocation.com/FDR
#https://github.com/carbocation/falsediscovery/blob/master/falsediscovery.goc(0.000003,0.004,0.001,0.1,0.05)
pV<-c(0.000003,0.004,0.001,0.1,0.05)
pV<-c(0.000719929,0.00139014,0.00162029,0.0146431,0.0147919,0.0263679,0.99,0.999)
p.adjust(pV,method="BH")
```
...
#https://cran.r-project.org/web/packages/proteomicsCV/proteomicsCV.pdf
not_log_intensity_df<-data.frame(intensity1=c(125000,100000000,983450,475987),intensity2=c(147000,230000000,720450,798656),intensity3=c(135000,156000000,853566,75036),intensity4=c(139000,134000000,250321,257986))
# base formula with raw intensity (no log transformation)
raw_cvs<-proteomicsCV::protCV(not_log_intensity_df)
#https://github.com/cran/proteomicsCV/pull/1
cvInt<-(apply(not_log_intensity_df,1,function(x) sd(x,na.rm=T)/mean(x,na.rm=T)))*100
summary(cvInt-raw_cvs)
log_intensity_df<-log2(not_log_intensity_df)
cvs<-proteomicsCV::protLogCV(log_intensity_df, 2)
plot(raw_cvs,cvs)
#https://github.com/cran/proteomicsCV/blob/master/R/proteomicsCV.R
#' @importFrom stats sd
#' @export
...
if(require("pheatmap")){
  grid::grid.draw(pheatmap::pheatmap(intensityLFQ,clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean",fontsize_row=6,cluster_cols=T,cluster_rows=T,fontsize_col=6)$gtable)
}
#logBAQ####
selection="iBAQ."
...
```{python}
#https://towardsdatascience.com/unravelling-complexity-a-novel-approach-to-manifold-learning-using-oise-injection-41251565fded
...
summary(warnings())
#Sys.setenv(R_USER="C:/Users/animeshs/")
#libPaths="C:/Users/animeshs/OneDrive/libR"
...
#plot
par(mfrow=c(1,2))
setwd("C:/Users/animeshs/OneDrive/Desktop/Scripts")
getwd()
...
```{r}
#NMDS https://rpubs.com/CPEL/NMDS ana basac fioccano
data‹-nmds [, 2:7)
...
```{r}
#Opening_DSS_Practice_Exercises
#install.packages("remotes")
...
## open practice exercises for chapter 2 of DSS...
```


```{r ESLR-CV}
#"Since the mean of many highly correlated quantities has higher variance than does the mean of many quantities that are not as highly correlated, the test error estimate resulting from LOOCV tends to have higher variance than does the test error estimate resulting from k-fold CV" https://www.statlearning.com/ file:///F:/OneDrive%20-%20NTNU/Desktop/ISLRv2_corrected_June_2023.pdf page 206
#https://hastie.su.domains/ISLR2/Labs/R_Labs/Ch5-resample-lab.R
#simulate https://cran.r-project.org/web/packages/faux/vignettes/rnorm_multi.html
#install.packages("faux")#library(faux)
dat <- faux::rnorm_multi(n = 1000, mu = c(0, 1, 2,3),sd = c(1),r = c(0.5), varnames = c("A","B","C","D"),empirical = FALSE)
plot(dat)
summary(dat)
meanCor=summary(dat)[4,1:ncol(dat)]
sd(as.numeric(sapply(strsplit(meanCor, ":"), "[[", 2)))
dat2 <- faux::rnorm_multi(n = 1000, mu = c(0, 1, 2,3),sd = c(1),r = c(0), varnames = c("A","B","C","D"),empirical = FALSE)
#dat2 <- faux::rnorm_multi(n = 1000, mu = c(0, 20, 20),sd = c(1, 5, 5),r = c(0, 0, 0), varnames = c("A","B","C"),empirical = FALSE)
plot(dat2)
summary(dat2)
meanCor2=summary(dat2)[4,1:ncol(dat2)]
sd(as.numeric(sapply(strsplit(meanCor2, ":"), "[[", 2)))
n=1000
sdCor=data.frame("cor"=c(1:n),"sdmean"=c(1:n))
for (i in 1:n){
  r=1/(i+1)
  dat <- faux::rnorm_multi(n = n, mu = c(0, 1, 2,3),sd = 1,r = r, varnames = c("A","B","C","D"),empirical = FALSE)
  meanCor=summary(dat)[4,1:ncol(dat)]
  sdCor[i,"cor"]=r
  sdCor[i,"sdmean"]=sd(as.numeric(sapply(strsplit(meanCor, ":"), "[[", 2)))
}
hist(sdCor[,2])
plot(sdCor[,2],sdCor[,1])
plot(log2(sdCor))
#https://strimmerlab.github.io/software/whitening/
#install.packages("whitening")
#library(whitening)
# https://strimmerlab.github.io/software/whitening/download/lusc.R https://doi.org/10.1186/s12859-018-2572-9
data(lusc)
X = lusc$rnaseq2
Y = lusc$methyl
...
corplot(cca.out, X, Y)
barplot(cca.out$lambda, xlab = "Dimension",ylab = "Canonical Correlations", names.arg = 1:length(cca.out$lambda))
loadplot(cca.out, 10)
...
B2 <- rbinom(N,size=1,prob=0.5)
D <- rnorm( N , 2*B2 + U + 0*M ) # change the 0 to turn on causal influence of mom
view rawRFDT1.R hosted with ❤ by GitHub
...
The causal influence of interest is defined by this intervention, written as p(D|do(M)), the distribution of D when we intervene on M.
...
Now that we have expressed the generative model this way, it automatically implies (by the axioms of probability) a joint probability distribution of all of the variables. The variables are the data (the observed variables) and the parameters (the latent variables).


# full-luxury bayesian inference https://github.com/rmcelreath/rethinking#quick-installation
library(rethinking)
library(cmdstanr)
...
inference for the effect m, b, and k. The rethinking package gives a concise summary of the marginal distributions of these variables with precis(flbi,pars=c("m","b","k")). The output is:
...
In this example, the causal query is so simple that there is a single parameter for it, m.

# simulate interventions

# B1 -> D (should be b*m as its linear)
...
quantile(d_D_B1)
view rawRFDT5.R hosted with ❤ by GitHub
The first chunk of code computes the distribution of the causal effect of B1 on D. The output is:

# version that marginalizes out the missing data https://gist.github.com/rmcelreath/6eb3c96ac52d53ae8f972e8fbf88e30d
  flbi_plus <- ulam(
    alist(
...
```{r sample}
#https://medium.com/evidentebm/sample-size-matters-a30e975a2f90
install.packages("stats")
library(stats)
# Calculate sample size for a two-sample t-test
power.t.test(d = 0.5, # effect size
             power = 0.8, # desired power
             sig.level = 0.05, # alpha level
             alternative = "two.sided") # two-sided test

# Install and import the statistics library
...
statistics.samplesize.ttest_ind_samplesize(0.5, # effect size
                                           ...
                                           power=0.8, # desired power
                                           alternative="two-sided") # two-sided test
Both of these examples will calculate the required sample size for each group in a two-sample t-test with an effect size of 0.5, an alpha level of 0.05, and a power of 0.8.

You can also use the samplesize module to calculate sample size for other types of statistical tests, such as ANOVA and regression. For example:

  # Calculate sample size for one-way ANOVA in Python
  statistics.samplesize.anova_oneway_samplesize(0.5, # effect size
                                                alpha=0.05, # alpha level
                                                power=0.8) # desired power
# Calculate sample size for regression in Python
statistics.samplesize.regression_samplesize(0.5, # effect size
                                            alpha=0.05, # alpha level
                                            power=0.8, # desired power
                                            p=2, # number of predictors
                                            ratio=1) # ratio of control to treatment group


```

```{r chartBB}
#https://rpubs.com/Mayank7j_2020/billboard_2020
# Target Web site URL
hot100page <- "https://www.billboard.com/charts/hot-100"
# Capture the HTML and XML level website information
#install.packages("xml2")
hot100 <- xml2::read_html(hot100page)
#"%>%" <- function(x,f) do.call(f,list(x))#https://www.datacamp.com/tutorial/pipe-r-tutorial
...
```{r PCA}
#https://medium.com/@gajendra.k.s/principal-component-analysis-pca-a190315de80b
data<-data.frame("M"=c(90,90,60,60,30),"E"=c(60,90,60,60,30),"A"=c(90,30,60,90,30))
biplot(princomp(data))
#https://www.marsja.se/how-to-calculate-z-score-in-r/
mData <- apply(data, 2, mean)
sData <- apply(data, 2, sd)
#dataZ <- sweep(data, 2, mData, FUN = "-") / sData
dataZ <- t((t(data)-(mData))/(sData))
biplot(princomp(dataZ))
dataCov<-cov(data)
biplot(princomp(dataCov))
MM<-apply(data["M"]*data["M"],2,mean)-mData["M"]*mData["M"]
ME<-apply(data["M"]*data["E"],2,mean)-mData["M"]*mData["E"]
MA<-apply(data["M"]*data["A"],2,mean)-mData["M"]*mData["A"]
EE<-apply(data["E"]*data["E"],2,mean)-mData["E"]*mData["E"]
EA<-apply(data["A"]*data["E"],2,mean)-mData["A"]*mData["E"]
AA<-apply(data["A"]*data["A"],2,mean)-mData["A"]*mData["A"]
dataC<-data.frame("MM"=c(MM,ME,MA),"EE"=c(ME,EE,EA),"AA"=c(MA,EA,AA))
biplot(princomp(dataC))
dataE<-eigen(dataC)
print(dataE$values)#comes in reverse of actual since it is auto sort to descending
print(dataE$vectors)
```

```{r}
#https://twitter.com/ATpoint90/status/1504515297683902467?t=KKSz6X4CEI9Ka_H2aO7svw&s=03
DESeq2::results() or the limma::treat() and edgeR::glmTreat() 
```

```{r excel}
#https://twitter.com/GuyProchilo/status/1506404403955310594?s=20&t=eViyk00dILRTyQiougk2mg
install.packages("datapasta")
val=datapasta::clipboard_context()
#c+s+T
val=data.frame(
             v1 = c(0.125679012,0.776539973,0.619025708,
                    0.32157613,0.669699637,0.220189265,0.728222934,
                    0.078949894,0.349956181,0.47484705,0.412066207,0.997157404,
                    0.040864736,0.005015682,0.741724793,0.985532228,0.481897076,
                    0.122803886,0.272411495,0.086328555),
             v2 = c(0.266959396,0.059405594,0.667290211,
                    0.612052542,0.998632215,0.790603446,0.615133562,
                    0.529131657,0.908355428,0.191942343,0.977932413,0.839793183,
                    0.238221414,0.013103873,0.769944087,0.763175287,
                    0.982618969,0.795093164,0.516149487,0.451133175)
   )
plot(val)
cor(val$v1,val$v2)
abline()
...
```{r}
#https://twitter.com/tjmahr/status/1504124329319096323?s=20&t=Uto1RSEpshj0o4cPLZG9jw
#why a mixed model (y ~ x + (x | g)) would be better than a linear model with (y ~ x * g)
```

```{r https://dpananos.github.io/posts/2023-09-24-l1/}
...
```{r http://freerangestats.info/blog/2023/09/23/circle}
...
```
```{r}
#https://twitter.com/PhilippBayer/status/1498505074082938881
plotly::ggplotly(tree) 
```
...
```
```{r}
#http://mixomics.org/case-studies/multilevel-vac18-case-study/
...
```{r multiTest}
#https://twitter.com/Amit_Levinson/status/1495485386226909189?t=VmWOPIhEVraRcxgogusWzA&s=03
library( dplyr )
drypurrr)
library(broom
mtcars_vars <- names(mtcars)
Generate possible formu las filter( Varl != Var2, Var2 != Var3, Varl != Var3) sample_n( 50) # Function to create fo rmu las f function ( Varl, Var2, Var3) paste( Varl, paste( Var2, Var3, sep ="+ "), sep = formulas <- mtcars_formulas
pmap_chr( f) head( formulas ) [1] "drat hp+ am" "mpg cyl + gsec" "drat hp disp" "wt am + qsec" "vs disp drat" "drat Carb+
# Iterate across the formu las and tidy the output with fo rmu la as name models <- map(formulas, data = mtcars ) ) set_names ( formulas)
map_dfr(tidy, id = ".x")
...
```{r featSelMS}
#https://github.com/tsunghengtsai/MSstats-feature-selection
```
```{r}
...
```{r ray}
#devtools::install_github("tylermorganwall/rayrender")#https://github.com/tylermorganwall/rayrender
...
vals = deSolve::ode(y = state, times = times, func = Lorenz, parms = parameters)[,-1]
#scale and rearrange:
vals = vals[,c(1,3,2)]/20
library(dplyr)
rayrender::generate_studio() %>% rayrender::add_object(rayrender::path(points=vals,y=-0.6,width=0.01,material=rayrender::diffuse(color="red"))) %>% rayrender::add_object(rayrender::sphere(y=5,z=5,radius=0.5,material=rayrender::light(intensity=200))) %>% rayrender::render_scene(samples=256,lookat=c(0,0.5,0),lookfrom=c(0,1,10),width = 800, height = 800, clamp_value=10,parallel=TRUE)
```

```{r corNetwork}
#https://github.com/smith-chem-wisc/MetaNetwork
BiocManager::install('WGCNA')
install.packages("ggdendro")
shiny::runGitHub("MetaNetwork", "smith-chem-wisc")
```
```{r}
#https://github.com/MarcIsak/MSLibrarian
devtools::install_github("MarcIsak/MSLibrarian")
```
...
dotR <- file.path("C:/Users/animeshs", ".R")
...
N <- nrow(y)
T <- ncol(y)
download.file("https://raw.githubusercontent.com/stan-dev/example-models/master/bugs_examples/vol1/rats/rats.stan","rats.stan")
rats_fit <- stan(file='rats.stan', data = list(N=N, T=T, y=y, x=x, xbar=xbar))
```
```{r cmdstanr}
#https://blog.mc-stan.org/2022/02/15/release-of-cmdstan-2-29/
#https://mc-stan.org/docs/2_29/stan-users-guide/linear-regression.html
...
Sys.getenv("R_USER")
Sys.setenv(R_USER="C:/Users/animeshs")
Sys.setenv(HOME="C:/Users/animeshs")
help(Startup)
...
#code https://github.com/animesh/rethinking and https://github.com/animesh/stat_rethinking_2022/tree/main
library(rethinking)
f <- alist(
    y ~ dnorm( mu , sigma ),
    mu ~ dnorm( 0 , 10 ),
...
#summary(fit)
precis(fit)
#lec https://www.youtube.com/watch?v=NSuTaeW6Orc https://github.com/animesh/stat_rethinking_2022 
#case-control bias x->y-z , model y~x+z is wrong,  y~x should work
f<-function(n=100,bXY=1,bYZ=1){
  X<-rnorm(n)
  Y<-rnorm(n,bXY*X)
  Z<-rnorm(n,bYZ*Y)
  bX<-coef(lm(Y~X))['X']
  bXZ<-coef(lm(Y~X+Z))['X']#wrong
  return(c(bX,bXZ))
}
sim<-mcreplicate(1e4,f(),mc.cores=1)#mc.cores=8
dens(sim[1,],lwd=3,xlab="post mu")
dens(sim[2,],lwd=3,col=2,add=TRUE)
cor(sim[1,],sim[2,])
sd(sim[1,])
cor(sim[1,],sim[2,])/sqrt(sd(sim[1,])*sd(sim[1,]))
plot(sim[1,],sim[2,])
abline(lm(sim[2,] ~ sim[1,]),col="red")
abline(lm(sim[1,] ~ sim[2,]),col="green")
lines(lowess(sim[1,],sim[2,]), col = "blue")
#X->Z->Y, u->Z, u->Y (confounder) https://youtu.be/NSuTaeW6Orc?t=3279
#post-treatment bias is common, "How Conditioning on Posttreatment Variables CanRuin Your Experiment and What to Do about It" https://onlinelibrary.wiley.com/doi/epdf/10.1111/ajps.12357 https://youtu.be/NSuTaeW6Orc?t=3494 
...
dens(sim[1,],lwd=3,xlab="post mu")
dens(sim[2,],lwd=3,col=2,add=TRUE)
#PSIS https://youtu.be/odGAAJDlgp8?list=PLDcUM9US4XdMROZ57-OIRtIK0aOynbgZN&t=2396
#AIC #PSIS https://youtu.be/odGAAJDlgp8?list=PLDcUM9US4XdMROZ57-OIRtIK0aOynbgZN&t=2396 but flat priors and large samples
#WAIC https://youtu.be/odGAAJDlgp8?list=PLDcUM9US4XdMROZ57-OIRtIK0aOynbgZN&t=2577 similar to PSIS but no-automatic diagnostics
#t-test https://youtu.be/odGAAJDlgp8?t=3253
fit <- quap( 
  alist(
...
```{r}
#https://docs.h2o.ai/driverless-ai/1-10-lts/docs/userguide/r_client_tutorial.html
library(dai)
...
sOBj<-MsBackendMgf::readMgf("L:/promec/Qexactive/LARS/2021/september/ole J/psoMSMS/210913_FT_CX_10MWCO_urt1_MS2_b.mgf")
library(Spectra)
...
```{r}
BiocManager::install(c('MSnbase', 'xcms', 'MAIT', 'ExpressionSetViewer'))
...
```
```{r}
data<-read.csv("L:/promec/TIMSTOF/LARS/2021/November/uncal/uncalMGF/cel.combinedScore.csv",row.names = 0)
data<-data[,-1]
boxplot(log2(data))
```
...
```{r score}
https://towardsdatascience.com/an-ultimate-guide-to-matching-and-propensity-score-matching-644395c46616
result_0 <- matchit(treat ~ age + educ + race + married +  nodegree + re74 + re75, data = lalonde, method = NULL, distance = 'glm')
...
shiny::runGitHub('gsarefdb', 'gsa-central')
```
```{r iceR}
...
```{r FS}
#https://twitter.com/thedataprof/status/1431996682858729477/photo/2
stats::prcomp
gifi::princals#CATPCA
FactoMineR::(M)CA
stats::cmdscale
ecodist::nmds
vegan::isomap
...
```
```{r treePlot}
#http://blog.phytools.org/2021/09/a-hacky-method-to-plot-states-of-one-or.html
#install.packages("phytools")
library(phytools)
#install.packages("geiger")
library(geiger)
lizard.tree<-read.nexus("lizard_tree.nex")
lizard.data<-read.csv("lizard_spines.csv",row.names=1,stringsAsFactors=TRUE)
head(lizard.data)
chk<-name.check(lizard.tree,lizard.data)
summary(chk)
lizard.tree<-drop.tip(lizard.tree,chk$tree_not_data)
habitat<-setNames(lizard.data$habitat,rownames(lizard.data))
tail.spines<-setNames(lizard.data$tail.spines,rownames(lizard.data))
...
```{r metaboanalyst}
#https://www.metaboanalyst.ca/MetaboAnalyst/faces/Secure/process/NormalizationView.xhtml
#I. Open the Metaboanalyst Link II. Data Type: Peak intensity table III. Format: Samples in column [Samples in column (Unpaired)] V. Click Submit
#Click on the Missing Value Estimation Step 1: Remove features with >30% missing values Step 2: Select “kNN (feature wise)” Skip the Data filtering by clicking “Proceed” Select “Quantile normalization” and “Log transformation” Click on Normalize Click on View result
...
mSet<-SanityCheckData(mSet)
mSet<-ContainMissing(mSet)
mSet<-RemoveMissingPercent(mSet, percent=0.3)
...
#Univariate Analysis Fold Change Analysis		T-tests		Volcano plot One-way Analysis of Variance (ANOVA) Correlation Heatmaps		Pattern Search		Correlation Networks (DSPC) Chemometrics Analysis Principal Component Analysis (PCA) Partial Least Squares - Discriminant Analysis (PLS-DA) Sparse Partial Least Squares - Discriminant Analysis (sPLS-DA) Orthogonal Partial Least Squares - Discriminant Analysis (orthoPLS-DA) Feature Identification Significance Analysis of Microarray (and Metabolites) (SAM) Empirical Bayesian Analysis of Microarray (and Metabolites) (EBAM) Cluster Analysis Hierarchical Clustering:		Dendrogram		Heatmaps Partitional Clustering:		K-means		Self Organizing Map (SOM) Classification & Feature Selection Random Forest Support Vector Machine (SVM)
